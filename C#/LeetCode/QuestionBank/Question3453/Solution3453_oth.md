### [三种方法：浮点二分 / 整数二分 / 差分（Python/Java/C++/Go）](https://leetcode.cn/problems/separate-squares-i/solutions/3076424/zheng-shu-er-fen-pythonjavacgo-by-endles-8yn5/)
...
设在水平线 $Y=y$ 下方的面积之和为 $area_y$，那么水平线上方的面积之和为 $totalArea-area_y$。

题目要求

$$area_y=totalArea-area_y$$
...
$$area_y=\sum\limits_{i=0}^{n-1}l_i\cdot min(max(y-y_i,0),l_i)$$
...
推荐的写法是固定一个**循环次数**，因为浮点数有舍入误差，可能算出的 $mid$ 和 $left$ 相等，此时 $left=mid$ 不会更新 $left$，导致死循环。

> **注**：本题由于值域小，也可以在 $left$ 和 $right$ 相距小于 $10^{-5}$ 时结束循环。但这种做法无法用于值域较大的场景，所以不推荐。
...
[本题视频讲解](https://leetcode.cn/l_ink/?target=https%3A%2F%2Fwww.bil_ibil_i.com%2Fvideo%2FBV1BRAGevERN%2F%3Ft%3D1m3s)，欢迎点赞关注~
...
- 时间复杂度：$O(n\log(U/\epsilon))$，其中 $n$ 是 $squares$ 的长度，$\epsilon =10^{-5}$，$U=max(y_i+l_i)$。
- 空间复杂度：$O(1)$。
...
设当前二分的整数值为 $multiY$，那么水平线下方的面积为

$$\begin{array}{rl} & l_i\cdot min(max(\dfrac{multiY}{M}-y_i,0),l_i) \\ = & \dfrac{l_i\cdot min(max(multiY-y_i\cdot M,0),l_i\cdot M)}{M}\end{array}$$

所以有

$$area_y=\dfrac{1}{M}\sum\limits_{i=0}^{n-1}l_i\cdot min(max(multiY-y_i\cdot M,0),l_i\cdot M)$$
...
$$2\sum\limits_{i=0}^{n-1}l_i\cdot min(max(multiY-y_i\cdot M,0),l_i\cdot M)\ge totalArea\cdot M$$
...
$$sumL=area_y-area_{y-1}$$

设答案为 $y^′$，那么

$$area_{y^′}=area_y-(y-y^′)\cdot sumL$$

题目要求

$$area_{y^′}\cdot 2=totalArea$$

解得

$$y^′=y-\dfrac{area_y-totalArea/2}{sumL}=y-\dfrac{area_y\cdot 2-totalArea}{sumL\cdot 2}$$
...
- 时间复杂度：$O(n\log U)$，其中 $n$ 是 $squares$ 的长度，$U=max(y_i+l_i)$。
- 空间复杂度：$O(1)$。

#### 方法三：差分+扫描线

![](./assets/img/Solution3453_oth_3.png)
...
$$area\cdot 2\ge totalArea$$

那么可以直接算出答案，计算公式和上面「方法二：整数二分（写法二）」是一样的。

$sumL$ 可以用**差分**维护，请看 [差分原理讲解](https://leetcode.cn/problems/car-pool_ing/solution/suan-fa-x_iao-ke-tang-chai-fen-shu-zu-fu-9d4ra/)。
...
- 时间复杂度：$O(n\log n)$，其中 $n$ 是 $squares$ 的长度。瓶颈在排序/维护有序集合上。
- 空间复杂度：$O(n)$。
