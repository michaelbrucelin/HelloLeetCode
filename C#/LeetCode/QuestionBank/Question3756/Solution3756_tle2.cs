...
{
    public class Solution3756_tle2 : Interface3756
    {
        /// <summary>
        /// 类前缀和，根据Solution3756_mle改进而来
        /// 记录[l, r]中非0数字和很简单，前缀和就可以
        /// 记录[l, r]中非0数字组合成的数字，不好办，这里先使用List<int>来模拟，稍后再优化
        /// 
        /// 依然TLE，参考测试用例04
        /// </summary>
        /// <param name="s"></param>
        ...
        public int[] SumAndMultiply(string s, int[][] queries)
        {
            ...
            for (int i = 0, l, r, sum; i < len; i++)
            {
                ...
                {
                    num = 0;
                    for (int j = nums[l]; j < nums[r + 1]; j++) num = (num * 10 + buffer[j]) % MOD;
