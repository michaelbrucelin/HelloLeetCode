### [我的日程安排表 I](https://leetcode.cn/problems/my-calendar-i/solutions/1643942/wo-de-ri-cheng-an-pai-biao-i-by-leetcode-nlxr/)
...
利用线段树，假设我们开辟了数组 $arr[0,\cdots,10^9]$，初始时每个元素的值都为 $0$，对于每次行程预订的区间 $[start,end)$ ，则我们将区间中的元素 $arr[start,\cdots,end-1]$ 中的每个元素都标记为 $1$，每次调用 $book$ 时，我们只需要检测 $arr[start,\cdots,end-1]$ 区间内是否有元素被标记为 $1$。实际我们不必实际开辟数组 $arr$，可采用动态线段树，懒标记 $lazy$ 标记区间 $[l,r]$ 已经被预订，$tree$ 记录区间 $[l,r]$ 的是否存在标记为 $1$ 的元素。

- 每次进行 $book$ 操作时，首先判断区间 $[start,\cdots,end-1]$ 是否存在元素被标记，如果存在被标记为 $1$ 的元素，则表明该区间不可预订；否则，则将可以预订。预订完成后，将 $arr[start,\cdots,end-1]$ 进行标记为 $1$，并同时更新线段树。
