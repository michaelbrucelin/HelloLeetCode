#### [方法一：状态压缩+动态规划](https://leetcode.cn/problems/maximize-grid-happiness/solutions/2316072/zui-da-hua-wang-ge-xing-fu-gan-by-leetco-0flf/)

**思路与算法**

为了方便描述，我们用 $iv$ 和 $ev$ 分别代表内向和外向的人数，并使用分数代指幸福感。

题目要求将 $ev + iv$ 个人分配到 $m \times n$ 的网格中，如果要考虑使用暴力搜索的方法遍历每一种可能，我们需要对搜索空间有一个初步的了解。在最差的情况下，需要将 $6$ 个内向的人和 $6$ 个外向的人分配到 $5 \times 5 = 25$ 的网格中，按照组合数学的计算方法，我们首先从 $25$ 个位置中选出 $6$ 个分配给内向的人，然后再从 $25 - 6 = 19$ 个位置中选出 $6$ 个分配给外向的人，总方案数为：
...
我们定义状态 $d(row, premask, iv, ev)$ 表示第 $row - 1$ 行的状态是 $premask$，并且 $row \sim m - 1$ 行最多还可以放置 $iv$ 个内向的人和 $ev$ 个外向的人时的分数。在转移过程中，我们需要考虑两部分的答案计算：
...
我们可以在开始求解之前，预先计算出所有状态的行内分数以及所有状态间的行间分数。具体来说，遍历所有长度为 $n$ 的三进制数 $mask$，计算行内分数以及 $mask$ 中内向的个数 $iv\_count[mask]$ 和外向的个数 $ev\_count[mask]$，过程中顺便将 $mask$ 的每一位预先存储到 $mask\_bits[mask]$ ，便于后续计算行间分数。

转移时，遍历 $row$ 行的状态 $mask$，有如下转移方程：

$$\begin{aligned} & d(row, premask, iv, ev) = \max \big\{ \\ & \qquad d(row + 1, mask, iv - iv\_count[mask], ev - ev\_count[mask]) \\ & \qquad + inner\_score[mask] + inter\_score[mask][premask] \\ \big\} \end{aligned} \}$$

其中 $inner\_score[mask]$ 表示 $mask$ 的行内分数，$inter\_score[mask][premask]$ 表示 $mask$ 和 $premask$ 的行间分数。

我们使用记忆化搜索的方式去实现，从 $d(0, 0, iv, ev)$ 开始求解，假设第 $0$ 行的 $premask$ 为 $0$，这样做可行的原因是状态为 $0$ 的行对分数没有贡献。当状态满足以下条件时，可以直接返回答案：

1.  当 $row = m$ 时，已经遍历完所有的行，**由于题目并不要求将所有的人都安排到网格中**，所以该状态合法，分数为 $0$。
2.  当 $iv = ev = 0$，表示后续已经没有人可以放置，该状态合法，分数为 $0$。
...
-   时间复杂度：$O((m\times iv \times ev + n) \times 3^{2n})$，其中 $m$ 是行数，$n$ 是列数，$iv$ 是内向的人数，$ev$ 是外向的人数。时间复杂度可以分为以下几个部分计算：
    -   预处理部分：计算行内分数时，遍历每一种状态的每一位，时间复杂度为 $O(n3^n)$。计算行间分数时，遍历每两种状态的每一位，时间复杂度为 $O(n3^{2n})$。因此这一部分的时间复杂度为 $O(n3^{2n})$。
    -   记忆化搜索部分：状态总数为 $O(m\times iv \times ev \times 3^n)$，每一次转移需枚举 $O(3^n)$ 次，转移的时间复杂度为 $O(1)$。因此这一部分的时间复杂度为 $O(m\times iv \times ev \times 3^{2n})$。
-   空间复杂度：$O(m\times iv \times ev \times 3^n + 3^{2n})$。可分为以下几个部分计算：
    -   预处理部分：存储三进制的每一位需要的空间为 $O(n3^n)$，存储行内分数、状态内内向的人的个数以及外向的人的个数需要的空间为 $O(3^n)$，存储行间分数需要的空间为 $O(3^{2n})$。这一部分的空间复杂度为 $O(3^{2n})$。
    -   记忆化搜索部分：搜索空间大小为 $O(m\times iv \times ev \times 3^n)$，即为递归和记忆化所需的空间。

#### [方法二：基于轮廓线的动态规划](https://leetcode.cn/problems/maximize-grid-happiness/solutions/2316072/zui-da-hua-wang-ge-xing-fu-gan-by-leetco-0flf/)

方法一中我们按行进行枚举，每一次需要枚举 $3^n$ 种状态进行转移。假如可以按位置进行枚举，只需考虑 $3$ 种状态。

如下图所示，假设 $n=5$, 我们目前枚举到了位置 $(x, y)$，其中 $0 \le x \lt m, 0 \le y \le n$，令 $pos = x \times n + y$。我们约定相邻元素之间的分数由下侧和右侧的元素负责计算，那么 $pos$ 需要计算其左侧 $pos - 1$ 和上侧 $pos - 5$ 处的相邻分数。接着继续遍历 $pos + 1$，需要计算它与 $pos$ 和 $pos - 4$ 之间的相邻分数，因此我们需要保留完整的一层状态（即完整的一个轮廓线，处于不同列的 $n$ 个元素），才可以保证后续的状态可以推算出具体代价。

![](./assets/img/Solution1659_off_01.png)

与方法一类似的，我们定义 $d(pos, mask, iv, ev)$ 为枚举到第 $pos$ 个位置，轮廓线状态为 $mask$，还有 $iv$ 个内向的人和 $ev$ 个外向的人未放置时的最大分数。其中，位置 $pos$ 编号从 $0$ 开始，到 $n \times m - 1$ 结束。$mask$ 的最低位表示 $pos - 1$ 的元素，最高位表示 $pos - n$ 的元素，如果在 $pos$ 位置上放置一个内向的人，轮廓线状态 $mask$ 将变为：$mask ~ \% ~ 3^{n - 1} \times 3 + 1$。

我们从 $d(0, 0, iv, ev)$ 进行搜索，当 $pos = n \times m$ 或者 $iv = ev = 0$ 时可以直接返回 $0$。转移过程中，还需要注意第 $0$ 行上方没有元素以及第 $0$ 列左侧没有元素的特殊情况，在本题中我们可以直接用 $0$ 表示没有元素。
...
-   时间复杂度：$O(m \times n \times iv \times ev \times 3^n)$，其中 $m$ 是行数，$n$ 是列数，$iv$ 是内向的人数，$ev$ 是外向的人数。预处理部分的时间复杂度为 $O(n)$，记忆化搜索的状态总数为 $O(m \times n \times iv \times ev \times 3^n)$，转移时需要枚举 $3$ 个状态，转移的时间复杂度为 $O(1)$，因此总体时间复杂度为 $O(m \times n \times iv \times ev \times 3^n)$。
-   空间复杂度：$O(m \times n \times iv \times ev \times 3^n)$。预处理部分的空间复杂度为 $O(n)$，记忆化搜索部分的空间复杂度为 $O(m \times n \times iv \times ev \times 3^n)$，因此总体空间复杂度为 $O(m \times n \times iv \times ev \times 3^n)$。
