### [猫和老鼠 II](https://leetcode.cn/problems/cat-and-mouse-ii/solutions/1473725/mao-he-lao-shu-ii-by-leetcode-solution-e5io/)
...
给定的网格包含 $rows$ 行和 $cols$ 列，网格中的单元格总数是 $total=rows \times cols$。每个单元格对应一个编号，第 $i$ 行第 $j$ 列的单元格编号是 $i \times cols+j$，其中 $0 \le i<rows$，$0 \le j<cols$。
...
- 时间复杂度：$O(rows^2 \times cols^2 \times (rows+cols))$，其中 $rows$ 和 $cols$ 分别是网格的行数和列数。状态数是 $O(rows^2 \times cols^2)$，对于每个状态需要 O(rows+cols) 的时间计算状态值，因此总时间复杂度是 $O(rows^2 \times cols^2 \times (rows+cols))$。
- 空间复杂度：$O(rows^2 \times cols^2)$，其中 $rows$ 和 $cols$ 分别是网格的行数和列数。需要记录每个状态的度和结果，状态数是 $O(rows^2 \times cols^2)$。
