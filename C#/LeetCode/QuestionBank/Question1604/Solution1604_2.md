#### [方法一：哈希表 + 排序](https://leetcode.cn/problems/alert-using-same-key-card-three-or-more-times-in-a-one-hour-period/solutions/2095526/jing-gao-yi-xiao-shi-nei-shi-yong-xiang-ioeiw/)
...
-   时间复杂度：$O(n \log n)$，其中 $n$ 是数组 $keyName$ 和 $keyTime$ 的长度。 需要遍历数组 $keyName$ 和 $keyTime$，得到每个员工的全部使用员工卡的时间，遍历的时间复杂度是 $O(n)$，存入哈希表的时间复杂度是 O(1)O(1)O(1)，因此时间复杂度是 $O(n)$。 然后判断每个员工是否收到系统警告，需要进行排序和遍历的操作，最坏情况下，排序的时间复杂度是 $O(n \log n)$，遍历的时间复杂度是 $O(n)$，因此时间复杂度是 $O(n \log n)$。 因此总时间复杂度是 $O(n \log n)$。
