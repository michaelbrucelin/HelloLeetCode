### [试填法 + 组合数学 + 优化（Python/Java/C++/Go）](https://leetcode.cn/problems/smallest-palindromic-rearrangement-ii/solutions/3649533/shi-tian-fa-zu-he-shu-xue-pythonjavacgo-qlu6e/)

#### 初步分析

由于 $s$ 是回文串，我们只需关心左半部分如何排列。

特别地，如果 $s$ 的长度是奇数，那么正中间的那个字母 $c$ 恰好出现奇数次，在重排后的回文串中，字母 $c$ 也出现奇数次，所以正中间的字母也必须是 $c$。
...
- 最左边能不能是字母 $a$？如果不能，试试字母 $b,c,\dots ,z$。
- 怎么判断能不能？假设最左边填字母 $a$，问题变成计算剩余位置的字符串的排列个数 $p$，如果 $p<k$，说明 $k$ 太大，继续尝试填字母 $b$；如果 $p\ge k$，说明右边足以容纳至少 $k$ 个排列，最左边就是字母 $a$。

如何计算字符串的排列个数？

#### 方法一（不适用？）

设剩余长度为 $sz$，如果随便排，有 $sz!$ 种方案。

当然，这里面有重复的，例如 $aabbb$，其中两个 $a$ 和三个 $b$ 的排列就是重复的，由于这两个 $a$ **无法区分**，三个 $b$ **无法区分**，方案数要除以 $2!3!$。

排列个数为

$$\dfrac{sz!}{\mathop{\Pi}\limits_{i=a}^{z}cnt_i!}$$
...
我们可以先从 $sz$ 个位置中，选 $2$ 个位置填字母 $a$，方案数为 $\binom{sz}{2}$。然后再从剩余 $sz-2$ 个位置中，选 $3$ 个位置填字母 $b$，方案数为 $\binom{sz-2}{3}$。依此类推。
...
具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1e3dBYLEDz%2F%3Ft%3D6m56s)，欢迎点赞关注$\sim$
...
- 时间复杂度：$O(n\vert \sum \vert (\vert \sum \vert +\log k))$，其中 $n$ 是 $s$ 的长度，$\vert \sum \vert =26$ 是字符集合的大小。计算 $perm$ 需要 $O(\vert \sum \vert +\log k)$ 的时间。注意是加法不是乘法，因为内层循环的 $res$ 至多乘以 $O(\log k)$ 个大于 $1$ 的数。
...
$$\dfrac{sz!}{\mathop{\Pi}\limits_{j=a}^{z}cnt_j!}$$

我们可以倒着填，从 $i=m-1$ 以及 $j=z$ 开始枚举，用上述公式计算已经填入的字母的排列个数。一旦排列个数 $\ge k$，就退出循环。
...
- 时间复杂度：$O(n\vert \sum \vert)$，其中 $n$ 是 $s$ 的长度，$\vert \sum \vert =26$ 是字符集合的大小。
- 空间复杂度：$O(n+\vert \sum \vert)$。

**注**：还可以用树状数组二分，进一步优化至 $O(n\log \vert \sum \vert)$。见评论。
