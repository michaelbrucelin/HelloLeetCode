#### [方法一：直接排序](https://leetcode.cn/problems/matrix-cells-in-distance-order/solutions/487123/ju-chi-shun-xu-pai-lie-ju-zhen-dan-yuan-ge-by-leet/)
...
-   时间复杂度：$O(rows \times cols \times \log(rows \times cols))$，存储所有点时间复杂度 $O(rows \times cols)$，排序时间复杂度 $O(rows \times cols \log(rows \times cols))$。
-   空间复杂度：$O(\log(rows \times cols))$，即为排序需要使用的栈空间，不考虑返回值的空间占用。

#### [方法二：桶排序](https://leetcode.cn/problems/matrix-cells-in-distance-order/solutions/487123/ju-chi-shun-xu-pai-lie-ju-zhen-dan-yuan-ge-by-leet/)
...
-   时间复杂度：$O(rows \times cols)$，存储所有点时间复杂度 $O(rows \times cols)$，桶排序时间复杂度 $O(rows \times cols)$。
-   空间复杂度：$O(rows \times cols)$，需要存储矩阵内所有点。

#### [方法三：几何法](https://leetcode.cn/problems/matrix-cells-in-distance-order/solutions/487123/ju-chi-shun-xu-pai-lie-ju-zhen-dan-yuan-ge-by-leet/)
...
![](./assets/img/Solution1030_off_3_01.png)
...
-   时间复杂度：$O\big((rows+cols)^2\big)$，我们需要遍历矩阵内所有点，同时也会遍历部分超过矩阵部分的点。在最坏情况下，给定的单元格位于矩阵的一个角，例如 $(0,0)$，此时最大的曼哈顿距离为 $rows+cols-2$，需要遍历的点数为 $2(rows+cols-2)(rows+cols-1)+1$，因此时间复杂度为 $O\big((rows+cols)^2\big)$。
