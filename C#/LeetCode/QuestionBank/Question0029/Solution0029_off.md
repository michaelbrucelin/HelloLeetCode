### [两数相除](https://leetcode.cn/problems/divide-two-integers/solutions/1041939/liang-shu-xiang-chu-by-leetcode-solution-5hic/)
...
因此，我们可以使用二分查找的方法得到 $Z$，即找出**最大**的 $Z$ 使得 $Z \times Y \le X$ 成立。
...
- 时间复杂度：$O(log⁡^2 C)$，其中 $C$ 表示 $32$ 位整数的范围。二分查找的次数为 $O(log⁡C)$，其中的每一步我们都需要 $O(log⁡C)$ 使用「快速乘」算法判断 $Z \times Y \le X$ 是否成立，因此总时间复杂度为 $O(log^2 C)$。
- 空间复杂度：$O(1)$。
