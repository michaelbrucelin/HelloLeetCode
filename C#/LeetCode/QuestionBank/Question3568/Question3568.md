...
给你一个 <code>m &times; n</code> 的网格图 `classroom`，其中一个学生志愿者负责清理散布在教室里的垃圾。网格图中的每个单元格是以下字符之一：
...
> - 收集所有垃圾的有效移动序列如下：
>   - 移动 1：从 `(0, 0)` &rightarrow; `(0, 1)`，消耗 1 单位能量，剩余 1 单位。
>   - 移动 2：从 `(0, 1)` &rightarrow; `(1, 1)`，收集垃圾 `'L'`。
...
> - 收集所有垃圾的有效移动序列如下：
>   - 移动 1：从 `(0, 1)` &rightarrow; `(0, 0)`，收集第一个垃圾 `'L'`，消耗 1 单位能量，剩余 3 单位。
>   - 移动 2：从 `(0, 0)` &rightarrow; `(1, 0)`，到达 `'R'` 重置区域，恢复能量为 4。
>   - 移动 3：从 `(1, 0)` &rightarrow; `(1, 1)`，收集第二个垃圾 `'L'`。
