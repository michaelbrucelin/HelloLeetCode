### [两种方法：分组循环 / 状态机 DP（Python/Java/C++/Go）](https://leetcode.cn/problems/trionic-array-ii/solutions/3741020/fen-zu-xun-huan-on-shi-jian-o1-kong-jian-ewr5/)
...
⚠**注意**：第一二段之间的峰顶 $peak$ 是第一二段共享的，第二三段之间的谷底 $bottom$ 是第二三段共享的。

该三段式子数组中的最大三段式子数组和，由三部分组成：

1. 必须包含从 $peak-1$ 到 $bottom+1$ 的所有元素。（每一段至少有两个数）
2. 从第一段的倒数第三个数（下标 $peak-2$）开始，往左的最大元素和。如果不存在则为 $0$。
3. 从第三段的第三个数（下标 $bottom+2$）开始，往右的最大元素和。如果不存在则为 $0$。

具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1BEh3zZEoM%2F%3Ft%3D37m15s)，欢迎点赞关注~
...
- 时间复杂度：$O(n)$，其中 $n$ 是 $nums$ 的长度。见 [分组循环](https://leetcode.cn/problems/longest-even-odd-subarray-with-threshold/solutions/2528771/jiao-ni-yi-ci-xing-ba-dai-ma-xie-dui-on-zuspx/)。对于本题，同一个元素可以在两个相交的极大三段式子数组中各遍历一次。
- 空间复杂度：$O(1)$。
...
假设 $nums[i]$ 是第三段的最后一个数，那么上一个数呢？

- $nums[i-1]$ 是第三段的倒数第二个数。问题变成 $nums[i-1]$ 作为第三段的最后一个数时的子数组最大和。
- $nums[i-1]$ 是谷底，即第二段的最后一个数。问题变成 $nums[i-1]$ 作为第二段的最后一个数时的子数组最大和。

假设 $nums[i]$ 是第二段的最后一个数（谷底），那么上一个数呢？

- $nums[i-1]$ 是第二段的倒数第二个数。问题变成 $nums[i-1]$ 作为第二段的最后一个数时的子数组最大和。
- $nums[i-1]$ 是峰顶，即第一段的最后一个数。问题变成 $nums[i-1]$ 作为第一段的最后一个数时的子数组最大和。

假设 $nums[i]$ 是第一段的最后一个数（峰顶），那么上一个数呢？

- $nums[i-1]$ 是第一段的倒数第二个数。问题变成 $nums[i-1]$ 作为第一段的最后一个数时的子数组最大和。
- $nums[i-1]$ 是第一段的第一个数。
...
- 子数组最后一个数的下标 $i$。
- $nums[i]$ 属于第 $j$ 段。其中 $j\in {1,2,3}$。

定义 $f[i][j]$ 表示 $nums[i]$ 作为第 $j$ 段的最后一个数时的子数组最大和。
...
**注**：从 $i=1$ 开始算。由于 $f[0][1]=-\infty $，所以 $f[1][1]=nums[0]+nums[1]$，这也保证了第一段至少有两个数。对于第二段来说，第一段的最后一个数也是第二段的第一个数，所以第二段也至少有两个数。对于第三段来说，第二段的最后一个数也是第三段的第一个数，所以第三段也至少有两个数。（如果 $f[i][3]\ne -\infty $，说明至少发生了一次 $f[i][3]=f[i-1][2]+nums[i]$ 的转移。）
