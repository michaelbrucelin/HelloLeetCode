### [根号分解算法（Python/Java/C++/Go）](https://leetcode.cn/problems/xor-after-range-multiplication-queries-ii/solutions/3755296/gen-hao-suan-fa-pythonjavacgo-by-endless-moxx/)

#### 算法一：暴力

暴力处理每个询问，把下标为 $l,l+k,l+2k,\dots $ 的数都乘以 $v$。

最坏情况每次需要 $O(\dfrac{n}{k})$ 的时间，整体 $O(\dfrac{nq}{k})$ 时间。其中 $n$ 是 $nums$ 的长度，$q$ 是 $queries$ 的长度。
...
处理每个询问只需要 $O(\log M)$ 时间计算逆元，其中 $M=10^9+7$。然而，我们需要遍历 $O(K)$ 个长为 $O(n)$ 的商分数组，总体需要 $O(nK+q\log M)$ 的时间。其中 $K$ 是 $ki$ 的最大值。
...
$$O(\dfrac{nq}{B}+nB+q\log M)$$

根据基本不等式，当 $B=q$ 时，上式取到最小值
...
[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1kTYyzwEDD%2F)，欢迎点赞关注~
...
- 时间复杂度：$O(n\sqrt{q}+q\log M)$，其中 $n$ 是 $nums$ 的长度，$q$ 是 $queries$ 的长度，$M=10^9+7$。
- 空间复杂度：$O(n\sqrt{q})$。
