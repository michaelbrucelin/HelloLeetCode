### [检测正方形](https://leetcode.cn/problems/detect-squares/solutions/1231129/jian-ce-zheng-fang-xing-by-leetcode-solu-vwzs/?envType=problem-list-v2&envId=ySsxoJfz)

#### 方法一：哈希表

**思路**

先考虑如何实现 `int count(int[] point)`，记输入的 $point$ 的横纵坐标分别为 $x$ 和 $y$。则形成的正方形的上下两条边中，其中一条边的纵坐标为 $y$，我们枚举另一条边的纵坐标为 $col$，则正方形的边长 $d$ 为 $\vert y-col\vert$ 且大于 $0$。有了其中一个点的坐标 $(x,y)$ 和一条横边的纵坐标 $col$，我们可以得到正方形的四个点的坐标分别为 $(x,y)$，$(x,col)$，$(x+d,y)$，$(x+d,col)$ 或 $(x,y)$，$(x,col)$，$(x-d,y)$，$(x-d,col)$。
