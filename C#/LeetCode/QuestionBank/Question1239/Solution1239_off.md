### [串联字符串的最大长度](https://leetcode.cn/problems/maximum-length-of-a-concatenated-string-with-unique-characters/solutions/834267/chuan-lian-zi-fu-chuan-de-zui-da-chang-d-g6gk/)
...
- 对于第 $pos$ 个数，我们有两种方法：选或者不选。如果 $mask$ 和 $masks[pos]$ 无公共元素，则可以选这个数，此时我们调用 $backtrack(pos+1,mask \vert  masks[pos])$ 进行递归。如果我们不选这个数，那么我们调用 $backtrack(pos+1,mask)$ 进行递归。
...
- 时间复杂度：$O(\vert \sum \vert +2^n)$。其中 $\vert \sum \vert$ 是 $arr$ 中所有字符串的长度之和，$n$ 是 $arr$ 的长度。遍历所有字符串需要 $O(\vert \sum \vert)$，回溯时由于每个元素有选或不选两种情况，最坏情况下会有 $2^n$ 种组合方式。因此总的时间复杂度为 $O(\vert \sum \vert +2^n)$。
- 空间复杂度：$O(n)$。

#### 方法二：迭代 + 位运算

我们可以遍历 $arr$，维护前 $i$ 个字符串构成的可行解集合，记作 $masks$。初始时，可行解集合仅包含一个空字符串。
...
- 时间复杂度：$O(\vert \sum \vert +2^n)$。其中 $\vert \sum \vert$ 是 $arr$ 中所有字符串的长度之和，$n$ 是 $arr$ 的长度。遍历所有字符串需要 $O(\vert \sum \vert)$，每次循环会将 $masks$ 的大小增加至多一倍，因此总的时间复杂度为 $O(\vert \sum \vert +2^0+2^1+...+2^{n-1})=O(\vert \sum \vert +2^n)$。
