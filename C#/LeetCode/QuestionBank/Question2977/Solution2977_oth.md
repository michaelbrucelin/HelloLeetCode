### [字典树+Floyd+记忆化搜索/递推（Python/Java/C++/Go）](https://leetcode.cn/problems/minimum-cost-to-convert-string-ii/solutions/2577877/zi-dian-shu-floyddp-by-endlesscheng-oi2r/)
...
3. 用 **Floyd 算法**求图中任意两点最短路，得到 $dis$ 矩阵，原理请看[【图解】带你发明 $Floyd$ 算法！](https://leetcode.cn/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance/solution/dai-ni-fa-ming-floyd-suan-fa-cong-ji-yi-m8s51/)这里得到的 $dis[i][j]$ 表示编号为 $i$ 的子串，通过若干次替换操作变成编号为 $j$ 的子串的最小成本。
4. 动态规划。定义 $dfs(i)$ 表示从 $source[i]$ 开始向后修改的最小成本。
5. 如果 $source[i]=target[i]$，可以不修改，$dfs(i)=dfs(i+1)$。
6. 也可以从 $source[i]$ 开始向后修改，利用字典树**快速判断** $source$ 和 $target$ 的下标从 $i$ 到 $j$ 的子串是否在 $original$ 和 $changed$ 中，如果在就用 $dis[x][y]+dfs(j+1)$ 更新 $dfs(i)$ 的最小值，其中 $x$ 和 $y$ 分别是 $source$ 和 $target$ 的这段子串对应的编号。
7. 递归边界 $dfs(n)=0$。
8. 递归入口 $dfs(0)$，即为答案。如果答案是无穷大则返回 $-1$。

[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1rG411k72D%2F)
...
也可以按照 [动态规划入门：从记忆化搜索到递推【基础算法精讲 17】](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1Xj411K7oF%2F)中讲的方法，$1:1$ 翻译成递推。$f[i]$ 的含义与 $dfs(i)$ 的含义是一样的。
...
- 时间复杂度：$O(n^2+mn+m^3)$，其中 $n$ 为 $source$ 的长度，$m$ 为 $cost$ 的长度。$DP$ 需要 $O(n^2)$ 的时间，把 $2m$ 个长度至多为 $n$ 的字符串插入字典树需要 $O(mn)$ 的时间，$Floyd$ 需要 $O(m^3)$ 的时间。
- 空间复杂度：$O(n+mn+m^2)$。DP 需要 $O(n)$ 的空间，把 $2m$ 个长度至多为 $n$ 的字符串插入字典树需要 $O(mn)$ 的空间，$Floyd$ 需要 $O(m^2)$ 的空间。
...
11. [链表、树与回溯（前后指针/快慢指针/DFS/BFS/直径/LCA）](https://leetcode.cn/circle/discuss/K0n^2gO/)
