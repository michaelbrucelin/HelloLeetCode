### [四叉树交集](https://leetcode.cn/problems/logical-or-of-two-binary-grids-represented-as-quad-trees/solutions/1668693/si-cha-shu-jiao-ji-by-leetcode-solution-wy1u/?envType=problem-list-v2&envId=tBJHVASZ)

#### 方法一：分治

**思路与算法**

题目给出两棵「四叉树」——$quadTree_1$，和 $quadTree_2$，它们分别代表一个 $n\times n$ 的矩阵，且每一个子节点都是父节点对应矩阵区域的 $\dfrac{1}{4}$ 区域：
...
我们需要把这两个矩阵中的对应位置的值进行「或」操作，然后返回操作后的矩阵即可。对于 $\forall x\in {0,1}$，有 $0 \vert  x=x$ 和 $1 \vert  x=1$ 成立。那么我们按照两棵树的对应的节点来进行合并操作，假设当前我们操作的两个节点分别为 $node_1$ 和 $node_2$，记节点的合并操作为 $node_1 \vert  node_2$：

1. $node_1$ 为叶子节点时：
    - 如果 $node_1$ 的值为 $1$，那么 $node_1 \vert  node_2=node_1$。
    - 否则 $node_1 \vert  node_2=node_2$。
2. $node_2$ 为叶子节点时：
    - 如果 $node_2$ 的值为 $1$，那么 $node_1 \vert  node_2=node_2$。
    - 否则 $node_1 \vert  node_2=node_1$。
...
- 时间复杂度：$O(n^2)$，其中 $n$ 是四叉树 $quadTree_1$ 和 $quadTree_2$ 对应矩阵的边长。最坏的情况下，整个矩阵都会被遍历。
- 空间复杂度：$O(\log n)$，其中 $n$ 是四叉树 $quadTree_1$ 和 $quadTree_2$ 对应矩阵的边长。空间开销主要为递归的空间开销。
