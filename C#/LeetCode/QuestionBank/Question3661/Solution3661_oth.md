### [教你一步步思考 DP：从记忆化搜索到递推到双指针优化（Python/Java/C++/Go）](https://leetcode.cn/problems/max_imum-walls-destroyed-by-robots/solutions/3762127/jiao-ni-yi-bu-bu-si-kao-dpcong-ji-yi-hua-dzd9/)
...
根据上面的讨论，定义状态为 $dfs(i,j)$，表示对于（排序后）下标在 $[0,i]$ 中的机器人，在机器人 $i+1$ 往左/右射击的前提下，能摧毁的最大墙壁数量。其中 $j=0$ 表示机器人 $i+1$ 往左射击，$j=1$ 表示机器人 $i+1$ 往右射击。

考虑机器人 $i$ 往哪个方向射击：

- 往左，那么接下来要解决的问题是，下标在 $[0,i-1]$ 中的机器人，在机器人 $i$ 往左射击的前提下，能摧毁的最大墙壁数量。即 $dfs(i-1,0)$。然后加上机器人 $i$ 摧毁的墙壁数量。
  - 往左最远为 $leftX=max(x_i-d_i,x_{i-1}+1)$，其中 $x_i$ 和 $d_i$ 分别表示机器人 $i$ 的位置和射击距离。为避免重复计算，我们规定，往左不能到达机器人 $i-1$。
  - 在 $walls$ 中二分查找 $\ge leftX$ 的第一个数的下标，记作 $left$。
  - 在 $walls$ 中二分查找 $\le x_i$ 的最后一个数的下标**加一**。根据 [二分查找 红蓝染色法【基础算法精讲 04】](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1AP41137w7%2F)，转化成二分查找 $\ge x_i+1$ 的第一个数的下标，记作 $cur_0$。
  - 那么 $[left,cur_0-1]$ 中的墙都能摧毁，这有 $cur_0-left$ 个。
- 往右，那么接下来要解决的问题是，下标在 $[0,i-1]$ 中的机器人，在机器人 $i$ 往右射击的前提下，能摧毁的最大墙壁数量。即 $dfs(i-1,1)$。
  - 往右最远为 $rightX=min(x_i+d_i,x_{i+1}-1)$ 或者 $min(x_i+d_i,x_{i+1}-d_{i+1}-1)$，取决于右边那个机器人是往右还是往左射击。
  - 在 $walls$ 中二分查找 $\le rightX$ 的最后一个数的下标**加一**，即 $\ge rightX+1$ 的第一个数的下标，记作 $right$。
  - 在 $walls$ 中二分查找 $\ge x_i$ 的第一个数的下标，记作 $cur_1$。
  - 那么 $[cur_1,right-1]$ 中的墙都能摧毁，这有 $right-cur_1$ 个。

这两种情况取最大值，就得到了 $dfs(i,j)$，即

$$dfs(i,j)=max(dfs(i-1,0)+cur_0-left,dfs(i-1,1)+right-cur_1)$$

**递归边界**：$dfs(-1,j)=0$。没有机器人，无法摧毁墙壁。

**递归入口**：$dfs(n-1,1)$。机器人 $n-1$ 右边没有机器人，等价于右边那个机器人往右射击。
...
具体请看视频讲解 [动态规划入门：从记忆化搜索到递推【基础算法精讲 17】](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1Xj411K7oF%2F)，其中包含把记忆化搜索 $1:1$ 翻译成递推的技巧。

[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1X9eJz2EWE%2F%3Ft%3D38m44s)，欢迎点赞关注~
...
- 时间复杂度：$O(n\log n+m\log m+n\log m)$，其中 $n$ 是 $robots$ 的长度，$m$ 是 $walls$ 的长度。由于每个状态只会计算一次，动态规划的时间复杂度 $=$ 状态个数 $\times $ 单个状态的计算时间。本题状态个数等于 $O(n)$，单个状态的计算时间为 $O(\log m)$，所以动态规划的时间复杂度为 $O(n\log m)$。前面排序需要 $O(n\log n+m\log m)$ 的时间。
- 空间复杂度：$O(n)$。保存多少状态，就需要多少空间。忽略排序的栈开销。

#### 四、1:1 翻译成递推

我们可以去掉递归中的「递」，只保留「归」的部分，即自底向上计算。

具体来说，$f[i][j]$ 的定义和 $dfs(i,j)$ 的定义是一样的，都表示对于（排序，添加哨兵后的）下标在 $[1,i]$ 中的机器人，在机器人 $i+1$ 往左/右射击的前提下，能摧毁的最大墙壁数量。

相应的递推式（状态转移方程）也和 $dfs$ 一样：

f[i][j]=max(f[i-1][0]+cur_0-left,f[i-1][1]+right-cur_1)

初始值 $f[0][j]=0$，翻译自（添加哨兵后的）递归边界 $dfs(0,j)=0$。

答案为 $f[n][1]$，翻译自（添加哨兵后的）递归入口 $dfs(n,1)$。
...
- 时间复杂度：$O(n\log n+m\log m+n\log m)$，其中 $n$ 是 $robots$ 的长度，$m$ 是 $walls$ 的长度。
- 空间复杂度：$O(n)$。忽略排序的栈开销。
...
类似 [背包问题](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV16Y411v7Y6%2F)，去掉 $f$ 的第一个维度，把 $f[i+1]$ 和 $f[i]$ 保存到**同一个数组**中。
...
由于随着 $i$ 变大，二分查找中的 $left,cur,right$ 也随之变大，我们可以用双指针（多指针）优化。这样算法瓶颈就在排序上了。
...
- 时间复杂度：$O(n\log n+m\log m)$，其中 $n$ 是 $robots$ 的长度，$m$ 是 $walls$ 的长度。瓶颈在排序上。
- 空间复杂度：$O(n)$。忽略排序的栈开销。
...
1. [滑动窗口与双指针（定长/不定长/单序列/双序列/三指针/分组循环）](https://leetcode.cn/circle/d_iscuss/0viNMK/)
...
7. [动态规划（入门/背包/状态机/划分/区间/状压/数位/数据结构优化/树形/博弈/概率期望）](https://leetcode.cn/circle/d_iscuss/tXLS3i/)
8. [常用数据结构（前缀和/差分/栈/队列/堆/字典树/并查集/树状数组/线段树）](https://leetcode.cn/circle/d_iscuss/mOr1u6/)
9. [数学算法（数论/组合/概率期望/博弈/计算几何/随机算法）](https://leetcode.cn/circle/d_iscuss/IYT3ss/)
