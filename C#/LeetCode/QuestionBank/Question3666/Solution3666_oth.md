### [两种方法：BFS / 数学（Python/Java/C++/Go）](https://leetcode.cn/problems/minimum-operations-to-equalize-binary-string/solutions/3768129/shu-xue-zuo-fa-pythonjavacgo-by-endlessc-ol6s/)
...
- 时间复杂度：$O(n\log n)$，其中 $n$ 是 $s$ 的长度。$[0,n]$ 中的每个数至多入队出队各一次，每次 $O(\log n)$ 时间。
- 空间复杂度：$O(n)$。
...
$$m\ge max(\lceil\dfrac{z}{k}\rceil ,\lceil\dfrac{z}{n-k}\rceil )$$
...
$$m\ge \lceil\dfrac{n-z}{n-k}\rceil$$
...
$$m\ge max(\lceil\dfrac{z}{k}\rceil,\lceil\dfrac{n-z}{n-k}\rceil)$$
...
具体来说，我们需要判断是否存在一个 $m$ 行 $n$ 列的 $0-1$ 矩阵 $M$，第 $i$ 行对应着第 $i$ 次操作，其中 $M_{i,j}=0$ 表示没有翻转 $s_j$，M_{i,j}=1 表示翻转 $s_j$。每一行的元素和都是 $k$，第 $j$ 列的元素和是 $s_j$ 的翻转次数 $a_j$。由于 $a_j\le m$ 且 $j\sum aj\le mk$，由 $Gale-Ryser$ 定理可得，这样的矩阵是存在的。
...
$$\lceil\dfrac{a}{b}\rceil =\lfloor\dfrac{a+b-1}{b}\rfloor$$
...
- 时间复杂度：$O(n)$，其中 $n$ 是 $s$ 的长度。瓶颈在遍历 $s$ 上。如果已知 $s$ 中的 $0$ 的个数，则时间复杂度是 $O(1)$。
- 空间复杂度：$O(1)$。
