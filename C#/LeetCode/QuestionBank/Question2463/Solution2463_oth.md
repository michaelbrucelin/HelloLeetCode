### [三种方法：记忆化搜索 -> 递推 -> 单调队列优化（Python/Java/C++/Go）](https://leetcode.cn/problems/minimum-total-distance-traveled/solutions/1951947/ji-yi-hua-sou-suo-by-endlesscheng-qctr/)

#### 一、贪心性质

看上去，一个工厂修理一段连续的机器人是最优的。为什么？

先排序，设工厂的位置为 $f_1<f_2<\dots <f_n$，机器人的位置为 $r_1<r_2<\dots <r_m$。

> **注**：题目保证工厂的位置互不相同，机器人的位置互不相同。

对于序列中的两个工厂 $f_a$ 和 $f_b$（a<b），两个机器人 $r_i$ 和 $r_j$（$i<j$），对比如下两个方案：

- $r_i$ 去 $f_a$，$r_j$ 去 $f_b$，移动距离之和为 $\vert r_i-f_a\vert +\vert r_j-f_b\vert$。
- $r_i$ 去 $f_b$，$r_j$ 去 $f_a$，移动距离之和为 $\vert r_i-f_b\vert +\vert r_j-f_a\vert$。

下图是其中一种情况。

![](./assets/img/Solution2463_oth.png)

用分类讨论可以证明，$\vert r_i-f_a\vert +\vert r_j-f_b\vert \le \vert r_i-f_b\vert +\vert r_j-f_a\vert$。
...
枚举工厂 $i$ 修 $k=0,1,2\dots ,min(j+1,limit_i)$ 个机器人，问题变成工厂下标区间 $[0,i-1]$ 修理机器人下标区间 $[0,j-k]$，机器人移动的最小总距离，即 $dfs(i-1,j-k)$，再加上机器人 $[j-k+1,j]$ 到工厂 $i$ 的距离。

对所有 $k$ 取最小值，就得到了 $dfs(i,j)$，即

$$dfs(i,j)=\min\limits_{k=0}^{min(j+1,limit_i)}\Bigg\{dfs(i-1,j-k)+\sum\limits_{p=j-k+1}^{j}\vert robot[p]-position[i]\vert\Bigg\}$$

由于 $k$ 每增加 $1$，距离和 $\sum\limits_{p=j-k+1}^{j}\vert robot[p]-position[i]\vert $ 就会新增一项 $\vert robot[j-k+1]-position[i]\vert $，所以可以用一个变量 $disSum$ 维护距离和，而不是对每个 $k$ 都跑一个循环算距离和。

**递归边界**：

- $dfs(i,-1)=0$。没有机器人了，总移动距离为 $0$。
- $dfs(-1,j)=\infty  (j\ge 0)$。没有工厂，但还有剩下的机器人，不合法。返回 $\infty $，这样上面公式中的 $min$ 不会取到不合法的情况。
...
- 时间复杂度：$O(nm^2+n\log n)$，其中 $n$ 是 $factory$ 的长度，$m$ 是 $robot$ 的长度。由于每个状态只会计算一次，动态规划的时间复杂度 $=$ 状态个数 $\times $ 单个状态的计算时间。本题状态个数等于 $O(nm)$，单个状态的计算时间为 $O(m)$，所以记忆化搜索的时间复杂度为 $O(nm^2)$。剩下的 $O(n\log n+m\log m)$ 是排序的时间复杂度，由于 $O(m\log m)$ 比 $O(nm^2)$ 小，可以省略。
...
$$f[i+1][j+1]=\min\limits_{k=0}^{min(limit_i,j+1)}\Bigg\{f[i][j-k+1]+\sum\limits_{p=j-k+1}^{j}\vert robot[p]-position[i]\vert\Bigg\}$$
...
$$f[i+1][j]=\min\limits_{k=0}^{min(limit_i,j)}\Bigg\{f[i][j-k]+\sum\limits_{p=j-k}^{j-1}\vert robot[p]-position[i]\vert\Bigg\}$$

这样可以少写很多 $+1$。

在 $k$ 从 $0$ 增大到 $min(limit_i,j)$ 的过程中，$j-k$ 从 $j$ 减小到 $max(j-limit_i,0)$。

把 $j-k$ 替换成 $k$，状态转移方程为：

$$f[i+1][j]=\min\limits_{k=max(j-limit_i,0)}^{j}\Bigg\{f[i][k]+\sum\limits_{p=k}^{j-1}\vert robot[p]-position[i]\vert\Bigg\}$$
...
- 时间复杂度：$O(nm^2+n\log n)$，其中 $n$ 是 $factory$ 的长度，$m$ 是 $robot$ 的长度。$DP$ 的时间复杂度为 $O(nm^2)$。剩下的 $O(n\log n+m\log m)$ 是排序的时间复杂度，由于 $O(m\log m)$ 比 $O(nm^2)$ 小，可以省略。
- 空间复杂度：$O(nm)$。

#### 六、单调队列优化

**前置题目**：[239\. 滑动窗口最大值](https://leetcode.cn/problems/sliding-window-maximum/)，视频讲解 [单调队列【基础算法精讲 27】](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1bM411X72E%2F)。

定义 $d_i[p]=\vert robot[p]-position[i]\vert $。
...
$$\begin{array}{rcl}f[i+1][j] & = & \min\limits_{k=max(j-limit_i,0)}^{j}\{f[i][k]+s_i[j]-s_i[k]\} \\ & = & s_i[j]+\min\limits_{k=max(j-limit_i,0)}^{j}\{f[i][k]-s_i[k]\}\end{array}$$

当 $j$ 增大时，$k$ 的范围是一个向右移动的滑动窗口，我们计算的是 $f[i][k]-s_i[k]$ 的**滑动窗口最小值**。上式的 $min$ 可以用**单调队列**优化至均摊 $O(1)$ 时间复杂度。
...
- 时间复杂度：$O(nm+n\log n+m\log m)$，其中 $n$ 是 $factory$ 的长度，$m$ 是 $robot$ 的长度。对于三重循环里面的二重循环，站在每个元素的视角看，这个元素在二重循环中最多入队出队各一次，因此二重循环的循环次数**之和**是 $O(m)$，所以三重循环的时间复杂度是 $O(nm)$。剩下的 $O(n\log n+m\log m)$ 是排序的时间复杂度。
- 空间复杂度：$O(nm)$。
