### [三种方法：线段树/树状数组/并查集（Python/Java/C++/Go）](https://leetcode.cn/problems/block-placement-queries/solutions/2790395/ping-heng-shu-xian-duan-shu-pythonjavacg-8klz/)

#### 方法一：正序回答询问+有序集合+线段树

![](./assets/img/Solution3161_oth.png)
...
如果二者的最大值 $\ge sz$，那么可以放入长为 $sz$ 的物块。
...
[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1SU411d7wj%2F%3Ft%3D7m56s)，欢迎点赞关注~
...
- 时间复杂度：$O(U+q\log U)$，其中 $q$ 是 $queries$ 的长度，$U$ 是 $x$ 的最大值。注意题目保证 $U\le 3q$。
- 空间复杂度：$O(U)$。
...
设 $x$ 左右最近元素分别为 $pre$ 和 $nxt$。

删除后，需要把 $d[nxt]$ 更新为 $nxt-pre$。注意这会让 $d[nxt]$ 增加。
...
需要维护两个并查集，分别记作 $left$ 和 $right$。

- 删除 $x$：把 $left$ 中的 $x$ 指向 $x-1$，把 $right$ 中的 $x$ 指向 $x+1$。如果要批量删除开区间 $(p,q)$ 中的元素 $x$，可以把 $left$ 中的 $x$ 指向 $p$，把 $right$ 中的 $x$ 指向 $q$。
- 查找 $x$ 的前驱：$x-1$ 在 $left$ 中的代表元。
- 查找 $x$ 的后继：$x+1$ 在 $right$ 中的代表元。如果 $x$ 已经删除，也可以查询 $x$ 在 $right$ 中的代表元。
