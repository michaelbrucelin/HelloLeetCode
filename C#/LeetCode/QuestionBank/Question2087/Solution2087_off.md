### [网格图中机器人回家的最小代价](https://leetcode.cn/problems/minimum-cost-homecoming-of-a-robot-in-a-grid/solutions/1127135/wang-ge-tu-zhong-ji-qi-ren-hui-jia-de-zu-62gy/)
...
- $(r,c)\rightarrow (r+1,c)\rightarrow (r+1,c+1)$：第一步移动到 $r+1$ 行，代价为 $rowCost[r+1]$；第二步移动到 $c+1$ 列，代价为 $colCost[c+1]$。总代价为 $rowCost[r+1]+colCost[c+1]$。
- $(r,c)\rightarrow (r,c+1)\rightarrow (r+1,c+1)$：第一步移动到 $c+1$ 列，代价为 $colCost[c+1]$；第二步移动到 $r+1$ 行，代价为 $rowCost[r+1]$。总代价为 $colCost[c+1]+rowCost[r+1]$。
...
不失一般性地，首先考虑从 $(r,c)$ 到 $(r+x,c)(x\ge 0)$ 的两种路径。一种路径为 $(r,c)\rightarrow (r+x,c)$，另一种路径为 $(r,c)\rightarrow (r,c+1)\rightarrow (r+x,c+1)\rightarrow (r+x,c)$。计算可得，后者相对于前者多出了 $colCost[c]+colCost[c+1]\ge 0$ 的总代价，亦即前者一定更优。
