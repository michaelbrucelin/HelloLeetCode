### [搜索推荐系统](https://leetcode.cn/problems/search-suggestions-system/solutions/101822/suo-tui-jian-xi-tong-by-leetcode-solution/)
...
- 时间复杂度：$O((\sum L+S)\times \log N)$，其中 $\sum L$ 是所有字符串的长度之和，$S$ 是字符串 `searchWord` 的长度，$N$ 是数组 `products` 的长度。对字符串进行排序的时间复杂度为 $O(\sum L\times \log N)$，二分查找进行了 $L$ 次，每次查找的时间复杂度为 $\log N$。虽然方法二的时间复杂度高于方法一，但方法二的常数较小，因此实际运行速度要快于方法一。
