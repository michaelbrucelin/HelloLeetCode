### [解数独](https://leetcode.cn/problems/sudoku-solver/solutions/414120/jie-shu-du-by-leetcode-solution/)
...
> 九宫格的范围为 $0 \le x \le 2$ 以及 $0 \le y \le 2$。
...
- 如果该位置是一个数字 $x$，那么我们需要将 $line[i][x-1]$，$column[j][x-1]$ 以及 $block[\lfloor i/3 \rfloor][\lfloor j/3 \rfloor][x-1]$ 均置为 $True$。

当我们结束了遍历过程之后，就可以开始递归枚举。当递归到第 $i$ 行第 $j$ 列的位置时，我们枚举填入的数字 $x$。根据题目的要求，数字 $x$ 不能和当前行、列、九宫格中已经填入的数字相同，因此 $line[i][x-1]$，$column[j][x-1]$ 以及 $block[\lfloor i/3 \rfloor][\lfloor j/3 \rfloor][x-1]$ 必须均为 $False$。
...
- 对于第 $i$ 行第 $j$ 列的位置，$line[i] \vert column[j] \vert block[\lfloor i/3 \rfloor][\lfloor j/3 \rfloor]$ 中第 $k$ 位为 $1$，表示该位置不能填入数字 $k+1$（因为已经出现过），其中 $\vert$ 表示按位或运算。如果我们对这个值进行 $\sim$ 按位取反运算，那么第 $k$ 位为 $1$ 就表示该位置可以填入数字 $k+1$，我们就可以通过寻找 $1$ 来进行枚举。由于在进行按位取反运算后，这个数的高位也全部变成了 $1$，而这是我们不应当枚举到的，因此我们需要将这个数和 $(111111111)_2 =(1FF)_{16}$ 进行按位与运算 $\&$，将所有无关的位置为 $0$；
