### [两种方法：二分答案 / Kruskal 算法（Python/Java/C++/Go）](https://leetcode.cn/problems/maximize-spanning-tree-stability-with-upgrades/solutions/3711009/liang-chong-fang-fa-er-fen-da-an-kruskal-6p7a/)
...
据此，可以**二分猜答案**。关于二分算法的原理，请看 [二分查找 红蓝染色法【基础算法精讲 04】](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1AP41137w7%2F)。

现在问题转化成一个判定性问题：

- 给定边权下界 $low$，能否得到一棵生成树，其边权都 $\ge$ low？

如果可以得到一棵生成树，说明答案 $\ge low$，否则说明答案 $<low$。
...
否则一定可以得到生成树，比如 $low=min(s_i)$ 的情况。

然后二分猜答案，处理上文说的判定性问题。

遍历 $edges$：

- 对于必选边，以及边权 $\ge low$ 的边，无需操作，用**并查集**合并。
- 其余边，如果剩余升级次数大于 $0$，且边权乘 $2$ 后 $\ge low$，且边的两个端点不在同一个连通块中，那么用**并查集**合并，同时把剩余升级次数减一。注意这些需要操作的边没有先后顺序，只要最终可以得到一棵生成树即可。
...
**问**：为什么二分结束后，答案 $ans$ 一定等于某个 $s_i$ 或者 2s_i？

**答**：反证法。假设 $ans$ 不等于任何 $s_i$ 或者 $2s_i$，这意味着生成树的最小边权大于 $ans$，也就是 $\ge ans+1$，换句话说，$check(ans+1)=true$。但根据循环不变量，二分结束后 $check(ans+1)=false$，矛盾。故原命题成立。

具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1j6gZzqEdc%2F%3Ft%3D37m18s)，欢迎点赞关注~
...
- 时间复杂度：$O((n+m\log n)\log U)$，其中 $m$ 是 $edges$ 的长度，$U=max(s_i)-min(s_i)$（常数系数可以忽略）。
- 空间复杂度：$O(n)$。

**注**：如果改成缩点建图 $+ DFS/BFS$ 判断是否连通，可以做到 $O((n+m)\log U)$。
...
- 时间复杂度：$O(m\log m+n+m\log n)$，其中 $m$ 是 $edges$ 的长度。瓶颈在排序上。
- 空间复杂度：$O(n)$。
