### [两种方法：前缀和 / 定长滑动窗口（Python/Java/C++/Go）](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-using-strategy/solutions/3755330/liang-chong-fang-fa-qian-zhui-he-ding-ch-uq98/)

#### 方法一：前缀和

计算两个前缀和数组：

- 定义数组 $c$，其中 $c[i]=prices[i]\cdot strategy[i]$。计算 $c$ 的前缀和，记作 $sum$。
...
1. $[0,i-k-1]$ 的 $prices[i]\cdot strategy[i]$ 之和，即 $sum[i-k]$。
2. $[i,n-1]$ 的 $prices[i]\cdot strategy[i]$ 之和，即 $sum[n]-sum[i]$。
3. $[i-k/2,i-1]$ 的 $prices[i]$ 之和，即 $sumSell[i]-sumSell[i-k/2]$。
...
[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1kTYyzwEDD%2F%3Ft%3D29m23s)，欢迎点赞关注~
...
设不修改时的利润为 $total$。修改后，利润（相比不修改）增加了 $sum$。所有窗口的 $sum$ 的最大值为 $maxSum$。那么答案为 $total+max(maxSum,0)$。这里可能出现 $maxSum<0$ 的情况，此时不修改更好，也就是与 $0$ 取最大值。

对于价格 $p$，如果修改前策略是 $x$，修改后策略是 $y$，那么利润增加了 $p\cdot (y-x)$。比如原来买入，现在持有（不买入），那么利润增加了 $p\cdot (0-(-1))=p$。又比如原来买入，现在卖出，那么利润增加了 $p\cdot (1-(-1))=2p$。

下面来计算每个窗口的 $sum$。考察从 $[i-k,i-1]$ 向右滑到 $[i-k+1,i]$，sum 如何变化。

先看窗口 $[i-k,i-1]$ 的 $sum$，分为左右两部分：

1. 左半为 $[i-k,i-k/2-1]$。修改前的策略为 $strategy[j]$，修改后的策略为 $0$，所以利润增加了 $prices[j]\cdot (-strategy[j])$ 之和，其中 $j$ 在左半中。
2. 右半为 $[i-k/2,i-1]$。修改前的策略为 $strategy[j]$，修改后的策略为 $1$，所以利润增加了 $prices[j]\cdot (1-strategy[j])$ 之和，其中 $j$ 在右半中。

当窗口向右滑动时，有三个位置的元素发生了变化：

1. $i$ 进入窗口（在右半），$sum$ 增加了 $prices[i]\cdot (1-strategy[i])$。
2. 下标为 $i-k/2$ 的元素从右半移到左半，交易策略从 $1$ 变成 $0$，所以 $sum$ 减少了 $prices[i-k/2]$。
3. $i-k$ 离开窗口（离开前在左半），$sum$ 减少了 $prices[i-k]\cdot (-strategy[i-k])$。
