...
给你一个细长的画，用数轴表示。这幅画由若干有重叠的线段表示，每个线段有 **独一无二** 的颜色。给你二维整数数组 `segments`，其中 <code>segments[i] = [start<sub>i</sub>, end<sub>i</sub>, color<sub>i</sub>]</code> 表示线段为 **半开区间** <code>[start<sub>i</sub>, end<sub>i</sub>)</code> 且颜色为 <code>color<sub>i</sub></code>。
...
你想要用 **最少数目** 不重叠 **半开区间** 来 **表示** 这幅混合颜色的画。这些线段可以用二维数组 `painting` 表示，其中 <code>painting[j] = [left<sub>j</sub>, right<sub>j</sub>, mix<sub>j</sub>]</code> 表示一个 **半开区间**<code>[left<sub>j</sub>, right<sub>j</sub>)</code> 的颜色 **和** 为 <code>mix<sub>j</sub></code>。
...
> ![](./assets/img/Question1943_01.png)
...
> ![](./assets/img/Question1943_02.png)
...
> ![](./assets/img/Question1943_03.png)
