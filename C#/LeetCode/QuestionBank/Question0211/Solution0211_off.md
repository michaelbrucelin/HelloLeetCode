### [添加与搜索单词 - 数据结构设计](https://leetcode.cn/problems/design-add-and-search-words-data-structure/solutions/1053383/tian-jia-yu-sou-suo-dan-ci-shu-ju-jie-go-n4ud/)

#### 方法一：字典树

**预备知识**

字典树（前缀树）是一种树形数据结构，用于高效地存储和检索字符串数据集中的键。前缀树可以用 $O(\vert S\vert)$ 的时间复杂度完成如下操作，其中 $\vert S\vert$ 是插入字符串或查询前缀的长度：

- 向字典树中插入字符串 $word$；
- 查询字符串 $word$ 是否已经插入到字典树中。

字典树的实现可以参考「[208\. 实现 $Trie$ (前缀树) 的官方题解](https://leetcode.cn/problems/implement-trie-prefix-tree/solution/shi-xian-trie-qian-zhui-shu-by-leetcode-ti500/)」。
...
- 时间复杂度：初始化为 $O(1)$，添加单词为 $O(\vert S\vert)$，搜索单词为 $O(\vert \sum \vert^{\vert S\vert})$，其中 $\vert S\vert$ 是每次添加或搜索的单词的长度，$\sum$ 是字符集，这道题中的字符集为全部小写英语字母，$\vert \sum \vert =26$。
  最坏情况下，待搜索的单词中的每个字符都是点号，则每个字符都有 $\vert \sum \vert$ 种可能。
- 空间复杂度：$O(\vert T\vert \cdot \vert \sum \vert)$，其中 $\vert T\vert$ 是所有添加的单词的长度之和，$\sum$ 是字符集，这道题中的字符集为全部小写英语字母，$\vert \sum \vert =26$。
