### [爱吃香蕉的珂珂](https://leetcode.cn/problems/koko-eating-bananas/solutions/1537914/ai-chi-xiang-jiao-de-ke-ke-by-leetcode-s-z4rt/?envType=problem-list-v2&envId=ySsxoJfz)
...
假设吃香蕉的速度是 $speed$，则当一堆香蕉的个数是 $pile$ 时，吃掉这堆香蕉需要 $\Big\lceil\dfrac{pile}{speed}\Big\rceil$ 小时，由此可以计算出吃掉所有香蕉需要的时间。如果在速度 $speed$ 下可以在 $h$ 小时内吃掉所有香蕉，则最小速度一定小于或等于 $speed$，因此将上界调整为 $speed$；否则，最小速度一定大于 $speed$，因此将下界调整为 $speed+1$。

二分查找结束之后，即可得到在 $h$ 小时内吃掉所有香蕉的最小速度 $k$。

实现方面，在计算吃掉每一堆香蕉的时间时，由于 $pile$ 和 $speed$ 都大于 $0$，因此 $\Big\lceil\dfrac{pile}{speed}\Big\rceil$ 等价于 $\Big\lfloor\dfrac{pile+speed-1}{speed}\Big\rfloor$。
