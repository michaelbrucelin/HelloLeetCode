### [可被 K 整除的最小整数](https://leetcode.cn/problems/smallest-integer-divisible-by-k/solutions/2263173/ke-bei-k-zheng-chu-de-zui-xiao-zheng-shu-ynog/)
...
题目要求出长度最小的仅包含的 $1$ 的并且被 $k$ 整除的正整数。我们从 $n=1$ 开始枚举，此时对 $k$ 取余得余数 $resid=1\mod k$。如果 $resid$ 不为 $0$，则表示 $n$ 当前还不能被 $k$ 整除，我们需要增加 $n$ 的长度。令 $n_{new}=n_{old}\times 10+1$，$resid_{new}=n_{new}\mod k$。将 $n_{old}$ 代入其中可得：

$$\begin{array}{rcl}resid_{new} & = & (n_{old}\times 10+1)\mod k \\ & = & ((n_{old}\mod k)\times 10+1)\mod k \\ & = & (resid_{old}\times 10+1)\mod k\end{array}$$

从上式可以发现，新的余数 $resid_{new}$ 可以由 $resid_{old}$ 推导得到。因此在遍历过程中不需要记录 $n$，只需记录 $resid$。由于 $resid$ 是对 $k$ 取余之后的余数，因此种类数不会超过 $k$。
...
$n-m$ 可以表示为 $11\dots 100\dots 0$ 的形式，因此有 $11\dots 100\dots 0\equiv 0(\mod k)$。

如果此时 $k$ 不为 $2$ 或 $5$ 的倍数，则 $k$ 与 $10$ 没有公因数，$k$ 与 $10$ 互质。$n-m$ 末尾的 $0$ 可以除掉，因此 $11\dots 1\equiv 0(\mod k)$，问题一定有解。
