### [检查网格中是否存在有效路径](https://leetcode.cn/problems/check-if-there-is-a-valid-path-in-a-grid/solutions/166473/jian-cha-wang-ge-zhong-shi-fou-cun-zai-you-xiao-lu/)
...
那么新的问题又来了，并查集中的点是一维的，而我们这里的单元格坐标是二维的，所以我们要做的第一步是把每一个单元格坐标映射成一个唯一的整数值，作为每个单元格的 $ID$。假设这里的单元格一共有 $m$ 行 $n$ 列，左上角为 $(0,0)$，右下角为 $(m-1,n-1)$，坐标 $(x,y)$ 可以这样映射到一个唯一的整数 $id$：id=x\times $n+y$。这样问题就得到了解决。
...
public class Solution {
    ...
    private void DetectL(int x, int y) {
        if (y - 1 >= 0 && (grid[x][y - 1] == 4 || grid[x][y - 1] == 6 || grid[x][y - 1] == 1)) {
            ds.Merge(GetId(x, y), GetId(x, y - 1));
        }
    }

    private void DetectR(int x, int y) {
        if (y + 1 < n && (grid[x][y + 1] == 3 || grid[x][y + 1] == 5 || grid[x][y + 1] == 1)) {
            ds.Merge(GetId(x, y), GetId(x, y + 1));
        }
    }

    private void DetectU(int x, int y) {
        if (x - 1 >= 0 && (grid[x - 1][y] == 3 || grid[x - 1][y] == 4 || grid[x - 1][y] == 2)) {
            ds.Merge(GetId(x, y), GetId(x - 1, y));
        }
    }

    private void DetectD(int x, int y) {
        if (x + 1 < m && (grid[x + 1][y] == 5 || grid[x + 1][y] == 6 || grid[x + 1][y] == 2)) {
            ds.Merge(GetId(x, y), GetId(x + 1, y));
        }
    }
...
DisjointSet* createDisjointSet(int m, int n) {
    DisjointSet* ds = (DisjointSet*)malloc(sizeof(DisjointSet));
    int size = m * n;
    ds->f = (int*)malloc(size * sizeof(int));
...
int find(DisjointSet* ds, int x) {
    ...
    ds->f[x] = find(ds, ds->f[x]);
    return ds->f[x];
}

void merge(DisjointSet* ds, int x, int y) {
    ds->f[find(ds, x)] = find(ds, y);
}
...
void detectL(int** grid, int m, int n, DisjointSet* ds, int x, int y) {
    if (y - 1 >= 0 && (grid[x][y - 1] == 4 || grid[x][y - 1] == 6 || grid[x][y - 1] == 1)) {
        merge(ds, getId(x, y, n), getId(x, y - 1, n));
    }
}

void detectR(int** grid, int m, int n, DisjointSet* ds, int x, int y) {
    if (y + 1 < n && (grid[x][y + 1] == 3 || grid[x][y + 1] == 5 || grid[x][y + 1] == 1)) {
        merge(ds, getId(x, y, n), getId(x, y + 1, n));
    }
}

void detectU(int** grid, int m, int n, DisjointSet* ds, int x, int y) {
    if (x - 1 >= 0 && (grid[x - 1][y] == 3 || grid[x - 1][y] == 4 || grid[x - 1][y] == 2)) {
        merge(ds, getId(x, y, n), getId(x - 1, y, n));
    }
}

void detectD(int** grid, int m, int n, DisjointSet* ds, int x, int y) {
    if (x + 1 < m && (grid[x + 1][y] == 5 || grid[x + 1][y] == 6 || grid[x + 1][y] == 2)) {
        merge(ds, getId(x, y, n), getId(x + 1, y, n));
    }
}

void handler(int** grid, int m, int n, DisjointSet* ds, int x, int y) {
    switch (grid[x][y]) {
        case 1:
            detectL(grid, m, n, ds, x, y);
            detectR(grid, m, n, ds, x, y);
            break;
        case 2:
            detectU(grid, m, n, ds, x, y);
            detectD(grid, m, n, ds, x, y);
            break;
        case 3:
            detectL(grid, m, n, ds, x, y);
            detectD(grid, m, n, ds, x, y);
...
bool hasValidPath(int** grid, int gridSize, int* gridColSize) {
    int m = gridSize;
    int n = gridColSize[0];
    DisjointSet* ds = createDisjointSet(m, n);

    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            handler(grid, m, n, ds, i, j);
        }
    }

    bool result = find(ds, getId(0, 0, n)) == find(ds, getId(m - 1, n - 1, n));
    freeDisjointSet(ds);
    return result;
}
```
...
function hasValidPath(grid: number[][]): boolean {
    ...
    const getId = (x: number, y: number): number => {
        return x * n + y;
    };

    const detectL = (x: number, y: number): void => {
        if (y - 1 >= 0 && (grid[x][y - 1] === 4 || grid[x][y - 1] === 6 || grid[x][y - 1] === 1)) {
            ds.merge(getId(x, y), getId(x, y - 1));
        }
    };

    const detectR = (x: number, y: number): void => {
        if (y + 1 < n && (grid[x][y + 1] === 3 || grid[x][y + 1] === 5 || grid[x][y + 1] === 1)) {
            ds.merge(getId(x, y), getId(x, y + 1));
        }
    };

    const detectU = (x: number, y: number): void => {
        if (x - 1 >= 0 && (grid[x - 1][y] === 3 || grid[x - 1][y] === 4 || grid[x - 1][y] === 2)) {
            ds.merge(getId(x, y), getId(x - 1, y));
        }
    };

    const detectD = (x: number, y: number): void => {
        if (x + 1 < m && (grid[x + 1][y] === 5 || grid[x + 1][y] === 6 || grid[x + 1][y] === 2)) {
            ds.merge(getId(x, y), getId(x + 1, y));
        }
    };
...
impl Solution {
    ...
    fn detect_l(grid: &Vec<Vec<i32>>, m: usize, n: usize, ds: &mut DisjointSet, x: usize, y: usize) {
        if y > 0 && (grid[x][y - 1] == 4 || grid[x][y - 1] == 6 || grid[x][y - 1] == 1) {
            ds.merge(Self::get_id(x, y, n), Self::get_id(x, y - 1, n));
        }
    }

    fn detect_r(grid: &Vec<Vec<i32>>, m: usize, n: usize, ds: &mut DisjointSet, x: usize, y: usize) {
        if y + 1 < n && (grid[x][y + 1] == 3 || grid[x][y + 1] == 5 || grid[x][y + 1] == 1) {
            ds.merge(Self::get_id(x, y, n), Self::get_id(x, y + 1, n));
        }
    }

    fn detect_u(grid: &Vec<Vec<i32>>, m: usize, n: usize, ds: &mut DisjointSet, x: usize, y: usize) {
        if x > 0 && (grid[x - 1][y] == 3 || grid[x - 1][y] == 4 || grid[x - 1][y] == 2) {
            ds.merge(Self::get_id(x, y, n), Self::get_id(x - 1, y, n));
        }
    }

    fn detect_d(grid: &Vec<Vec<i32>>, m: usize, n: usize, ds: &mut DisjointSet, x: usize, y: usize) {
        if x + 1 < m && (grid[x + 1][y] == 5 || grid[x + 1][y] == 6 || grid[x + 1][y] == 2) {
            ds.merge(Self::get_id(x, y, n), Self::get_id(x + 1, y, n));
        }
    }
...
```CSharp
public class Solution {
    private const int MAX_N = 300 * 300 + 5;
    private static readonly int[] patterns = { 0, 0b1010, 0b0101, 0b1100, 0b0110, 0b1001, 0b0011 };
...
func hasValidPath(grid [][]int) bool {
    ...
    patterns := []int{0, 0b1010, 0b0101, 0b1100, 0b0110, 0b1001, 0b0011}
...
```C
#define MAX_N (300 * 300 + 5)

const int patterns[7] = {0, 0b1010, 0b0101, 0b1100, 0b0110, 0b1001, 0b0011};
...
bool hasValidPath(int** grid, int gridSize, int* gridColSize) {
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            int pattern = patterns[grid[i][j]];
            for (int d = 0; d < 4; ++d) {
                if (pattern & (1 << d)) {
                    int sx = i + dirs[d][0];
                    int sy = j + dirs[d][1];
                    if (sx >= 0 && sx < m && sy >= 0 && sy < n &&
                        (patterns[grid[sx][sy]] & (1 << ((d + 2) % 4)))) {
                        merge(&ds, getId(i, j), getId(sx, sy));
                    }
                }
            }
        }
    ...
    return find(&ds, getId(0, 0)) == find(&ds, getId(m - 1, n - 1));
}
```
...
function hasValidPath(grid: number[][]): boolean {
    const MAX_N = 300 * 300 + 5;
    const patterns: number[] = [0, 0b1010, 0b0101, 0b1100, 0b0110, 0b1001, 0b0011];
...
impl Solution {
    pub fn has_valid_path(grid: Vec<Vec<i32>>) -> bool {
        const MAX_N: usize = 300 * 300 + 5;
        const PATTERNS: [i32; 7] = [0, 0b1010, 0b0101, 0b1100, 0b0110, 0b1001, 0b0011];
        const DIRS: [[i32; 2]; 4] = [
            [-1, 0],
            [0, 1],
            [1, 0],
        ...
        let get_id = |x: usize, y: usize| -> usize {
            x * n + y
        };

        for i in 0..m {
            for j in 0..n {
                let pattern = PATTERNS[grid[i][j] as usize];
                for d in 0..4 {
                    if (pattern & (1 << d)) != 0 {
                        let sx = i as i32 + DIRS[d][0];
                        let sy = j as i32 + DIRS[d][1];
                        if sx >= 0 && sx < m as i32 && sy >= 0 && sy < n as i32 {
                            let sx = sx as usize;
                            let sy = sy as usize;
                            let neighbor_pattern = PATTERNS[grid[sx][sy] as usize];
                            if (neighbor_pattern & (1 << ((d + 2) % 4))) != 0 {
                                ds.merge(get_id(i, j), get_id(sx, sy));
                            }
                        }
                    }
                }
        ...
        ds.find(get_id(0, 0)) == ds.find(get_id(m - 1, n - 1))
    }
}
```
