### [函数的独占时间](https://leetcode.cn/problems/exclusive-time-of-functions/solutions/1725969/han-shu-de-du-zhan-shi-jian-by-leetcode-d54e2/)
...
- 时间复杂度：$O(n)$，其中 $n$ 为全部日志 $\log s$ 的数量，$n$ 条日志信息对应总共 $n$ 次入栈和出栈操作。
- 空间复杂度：$O(n)$，其中 $n$ 为全部日志 $\log s$ 的数量，$n$ 条日志信息对应 $2n$ 次入栈操作，最坏的情况下全部 $2n$ 条日志入栈后才会依次弹栈。
