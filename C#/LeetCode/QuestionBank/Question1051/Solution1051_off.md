### [高度检查器](https://leetcode.cn/problems/height-checker/solutions/1593917/gao-du-jian-cha-qi-by-leetcode-solution-jeb0/)
...
待排序完成后，我们统计 $heights[i] \neq expected[i]$ 的下标数量即可。
...
-   时间复杂度：$O(n \log n)$，其中 $n$ 是数组 $heights$ 的长度。即为排序需要的时间。
-   空间复杂度：$O(n)$，即为数组 $expected$ 需要的空间。
