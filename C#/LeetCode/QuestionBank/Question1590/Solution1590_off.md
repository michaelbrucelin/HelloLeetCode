### [使数组和能被 P 整除](https://leetcode.cn/problems/make-sum-divisible-by-p/solutions/2157277/shi-shu-zu-he-neng-bei-p-zheng-chu-by-le-dob9/)

#### 方法一：前缀和

> 定理一：给定正整数 $x$、$y$、$z$、$p$，如果 $y\bmod p=x$，那么 $(y-z)\bmod p=0$ 等价于 $z\bmod p=x$。
>
> 证明：$y\bmod p=x$ 等价于 $y=k_1\times p+x$，$(y-z)\bmod p=0$ 等价于 $y-z=k_2\times p$，$z\bmod p=x$ 等价于 $z=k3\times p+x$，其中 $k_1$、$k_2$、$k_3$ 都是整数，那么给定 $y=k_1\times p+x$，有 $y-z=k_2\times p\leftrightarrow z=(k_1-k_2)\times p+x\leftrightarrow z=k_3\times p+x$。
