...
- 对于 $dec$ 操作，测试用例保证 $key$ 在链表中。
  ...
  - 若 $key$ 出现不止一次：设 $key$ 所在节点为 $cur$，若 $cur.prev$ 为空或 $cur.prev.count<cur.count-1$，则先插入一个 $count=cur.count-1$ 的新节点至 $cur$ 之前，然后将 $key$ 插入到 $cur.prev.keys$ 中。更新 $nodes$ 中 $key$ 所处的节点。
...
- 空间复杂度：$O(I)$，其中 $I$ 是调用 $inc$ 的次数。最坏情况下每次调用 $inc$ 传入的字符串均不相同，我们需要 $O(I)$ 大小的哈希表来存储所有字符串。
