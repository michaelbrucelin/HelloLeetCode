### [随机数索引](https://leetcode.cn/problems/random-pick-index/solutions/1444589/sui-ji-shu-suo-yin-by-leetcode-solution-ofsq/)
...
> 该方法时间复杂度较高，无法通过测试用例。在这里写这种方法是为了介绍一下使用时间换取空间的方法。
...
$$\begin{array}{cl}
    ...
    = & P(第 i 次随机选择的值=0)\times P(第 i+1 次随机选择的值\ne 0)\times \dots \times P(第 k 次随机选择的值\ne 0) \\
    = & \dfrac{1}{i}\times (1-\dfrac{1}{i+1})\times \dots \times (1-\dfrac{1}{k}) \\
    = & \dfrac{1}{i}\times \dfrac{i}{i+1}\times \dots \times \dfrac{k-1}{k} \\
    = & \dfrac{1}{k}
\end{array}$$
