### [最长重复子数组](https://leetcode.cn/problems/maximum-length-of-repeated-subarray/solutions/310099/zui-chang-zhong-fu-zi-shu-zu-by-leetcode-solution/?envType=problem-list-v2&envId=ySsxoJfz)
...
![](./assets/img/Solution0718_off_2.gif)
...
$$hash(S)=\sum\limits_{i=0}^{\vert S\vert -1}base^{\vert S\vert -(i+1)}\times S[i]$$

形象地说，就是把 `S` 看成一个类似 `base` 进制的数（左侧为高位，右侧为低位），它的十进制值就是这个它的哈希值。由于这个值一般会非常大，因此会将它对另一个素数 `\bmod ` 取模。
...
$$hash(S[1:len+1])=(hash(S[0:len])-base^{len-1}\times S[0])\times base+S[len]$$
