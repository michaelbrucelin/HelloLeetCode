### [两种方法：枚举中间 / 一次遍历（Python/Java/C++/Go）](https://leetcode.cn/problems/count-special-triplets/solutions/3700554/mei-ju-zhong-jian-qian-hou-zhui-fen-jie-5uad8/)
...
- 在 $[0,j-1]$ 中，$nums[j]\cdot 2$ 的出现次数。
- 在 $[j+1,n-1]$ 中，$nums[j]\cdot 2$ 的出现次数。
...
由于答案不超过 $n\cdot 105\cdot 105\le 1015$，可以只在返回时取模。

[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1qsMxz6EEd%2F%3Ft%3D9m46s)，欢迎点赞关注~
...
- 有多少个二元组 $(i,j)$，满足 $i<j<k$ 且 $nums[i]=x$ 且 $nums[j]=2x$。用哈希表 $cnt12$ 记录这样的二元组个数。

这个问题也可以枚举右维护左，即枚举 $j$，问题变成：

- 在 $j$ 左边有多少个数等于 $nums[j]\cdot$ 2？用哈希表 $cnt1$ 记录。
