### [排序之后 BFS（Python/Java/C++/Go）](https://leetcode.cn/problems/multi-source-flood-fill/solutions/3954669/pai-xu-zhi-hou-bfspythonjavacgo-by-endle-4mp0/?envType=problem-list-v2&envId=ySsxoJfz)

本题是多源 $BFS$ 问题，关键是题目的这个要求：如果多个颜色在同一时间步到达同一个未着色单元格，该单元格将采用具有**最大值**的颜色。

我们可以先把 $sources$ 按照 $color$ **从大到小排序**，然后把 $sources$ 中的所有元素入队。这样在 $BFS$ 的过程中，我们会优先扩散颜色大的单元格，这些单元格的邻居会先入队。

[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1agddBJEnQ%2F%3Ft%3D7m45s)，欢迎点赞关注~
...
- 时间复杂度：$O(k\log k+nm)$，其中 $k$ 是 $sources$ 的长度。
- 空间复杂度：$O(nm)$。
