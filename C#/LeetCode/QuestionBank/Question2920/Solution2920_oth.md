### [树形 DP：记忆化搜索 / 递推（Python/Java/C++/Go）](https://leetcode.cn/problems/maximum-points-after-collecting-coins-from-all-nodes/solutions/2503152/shu-xing-dp-ji-yi-hua-sou-suo-by-endless-phzx/)
...
$$dfs(i,j)=max \begin{cases}(coins[i] >> j)-k+ \sum_{ch}dfs(ch,j) \\ (coins[i] >> (j+1))+ \sum_{ch}dfs(ch,j+1) \end{cases}$$
...
此外，为避免错把父亲当作儿子，可以额外传入 $f_a$ 表示父节点，遍历 $i$ 的邻居时，跳过邻居节点是 $f_a$ 的情况。

关于记忆化搜索的原理，请看视频讲解 [动态规划入门：从记忆化搜索到递推](https://leetcode.cn/link/?target=https%3A%2F%2Fb23.tv%2F72onpYq)。

[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1tw411q7VZ%2F%3Ft%3D45m18s)
...
- 时间复杂度：$O(n \log U)$，其中 $n$ 是 $coins$ 的长度，$U=max(coins)$。
- 空间复杂度：$O(n \log U)$。
...
去掉参数 $j$，改成每个节点 $i$ 返回一个长为 $14$ 的列表 $f_i$，其中 $f_i[j]$ 对应上面 $dfs(i,j)$ 的计算结果。

递推式为

$$f_i[j]=max \begin{cases}(coins[i] >> j)-k+ \sum_{ch}f_{ch}[j] \\ (coins[i] >> (j+1))+ \sum_{ch}f_{ch}[j+1] \end{cases}$$

把 $\sum_{ch}f_{ch}[j]$ 累加到 $s[j]$ 中，上式为

$$f_i[j]=max \begin{cases}(coins[i] >> j)-k+s[j] \\ (coins[i] >> (j+1))+s[j+1] \end{cases}$$

特判 $j=13$ 的情况，上式为

$$f_i[13]=(coins[i] >> 13)-k+s[13]$$
