### [两种方法：贡献法 / 增量法（Python/Java/C++/Go）](https://leetcode.cn/problems/total-appeal-of-a-string/solutions/1461618/by-endlesscheng-g405/?envType=problem-list-v2&envId=ySsxoJfz)

#### 方法一：贡献法

考虑每个 $s[i]$ 的贡献。

对于重复字符，人为规定，**只考虑第一个字符的贡献**。比如子串中有多个字母 $a$，我们只计算其中第一个字母 $a$ 的贡献。

设 $s[i]$ 上一次出现的下标为 $last[s[i]]$。如果不存在则为 $-1$。

那么子串的左端点可以从 $last[s[i]]+1$ 到 $i$，子串的右端点可以从 $i$ 到 $n-1$。
...
- 如果 $s[i]$ 之前遇到过，设其上次出现的下标为 $j$，那么向子串 $s[0..i-1], s[1..i-1], s[2..i-1],\dots ,s[j..i-1]$ 的末尾添加 $s[i]$ 后，这些子串的引力值是不会变化的，因为 $s[i]$ 已经在 $s[j]$ 处出现过了；而子串 $s[j+1..i-1], s[j+2..i-1],\dots ,s[i-1..i-1]$ 由于不包含字符 $s[i]$，这些子串的引力值都会增加 $1$，因此有 $i-j-1$ 个子串的引力值会增加 $1$，这些子串的引力值之和会增加 $i-j-1$，再加上 $1$，即 $s[i]$ 单独组成的子串的引力值。
