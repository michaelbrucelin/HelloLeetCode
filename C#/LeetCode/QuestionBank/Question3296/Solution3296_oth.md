### [两种方法：最小堆模拟/二分答案（Python/Java/C++/Go）](https://leetcode.cn/problems/minimum-number-of-seconds-to-make-mountain-height-zero/solutions/2925848/er-fen-da-an-pythonjavacgo-by-endlessche-myg4/?envType=problem-list-v2&envId=tBJHVASZ)

#### 方法一：最小堆模拟

循环 $mountainHeight$ 次，每次选一个「工作后总用时」最短的工人，把山的高度降低 $1$。

具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1WRtDejEjD%2F)，欢迎点赞关注~
...
- 时间复杂度：$O(mountainHeight\log n)$，其中 $n$ 是 $workerTimes$ 的长度。
- 空间复杂度：$O(n)$。
...
- 每个工人**至多**花费 $m$ 秒，总共降低的高度是多少？能否大于等于 mountainHeight？

遍历 $workerTimes$，设 $t=workerTimes[i]$，那么有

$$t+2t+\dots +xt=t\cdot\dfrac{x(x+1)}{2}\le m$$

即

$$\dfrac{x(x+1)}{2}\le \Bigl\lfloor\dfrac{m}{t}\Bigr\rfloor =k$$
...
$$\Bigl\lfloor\dfrac{-1+\sqrt{1+8k}}{2}\Bigr\rfloor =\Bigl\lfloor\dfrac{-1+\lfloor\sqrt{1+8k}\rfloor}{2} \Bigr\rfloor $$

累加上式，如果和 $\ge mountainHeight$，则说明答案 $\le m$，否则说明答案 $>m$。

最后，讨论二分的上下界。这里用开区间二分，其他二分写法也是可以的。

- 开区间二分下界：$0$，无法把山的高度降低到 $0$。
- 开区间二分上界：设 $maxT$ 为 $workerTimes$ 的最大值，假设每个工人都是最慢的 $maxT$，那么单个工人要把山降低 $h=\Bigl\lceil\dfrac{mountainHeight}{n}\Bigr\rceil $，耗时 $maxT\cdot (1+2+\dots +h)=maxT\cdot\dfrac{h(h+1)}{2}$，将其作为开区间的二分上界，一定可以把山的高度降低到 $\le 0$。

关于上取整的计算，当 $a$ 和 $b$ 均为正整数时，我们有

$$\Bigl\lceil\dfrac{a}{b}\Bigr\rceil =\Bigl\lfloor\dfrac{a-1}{b}\Bigr\rfloor +1$$

证明见 [上取整下取整转换公式的证明](https://leetcode.cn/link/?target=https%3A%2F%2Fzhuanlan.zhihu.com%2Fp%2F1890356682149838951)。
...
- 时间复杂度：$O(n\log U)$，其中 $n$ 是 $workerTimes$ 的长度，$U\le 5\cdot 10^{10}(10^5+1)$ 是二分上界。二分 $O(\log U)$ 次，每次 $O(n)$ 时间。开平方有专门的 $CPU$ 指令，可以视作 $O(1)$。
- 空间复杂度：$O(1)$。
