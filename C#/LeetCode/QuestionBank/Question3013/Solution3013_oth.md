### [两个有序集合维护前 k-1 小（Python/Java/C++/Go）](https://leetcode.cn/problems/divide-an-array-into-subarrays-with-minimum-cost-ii/solutions/2614067/liang-ge-you-xu-ji-he-wei-hu-qian-k-1-xi-zdzx/)

第一段的第一个数是确定的，即 $nums[0]$。

如果知道了第二段的第一个数的位置（记作 $p$），第三段的第一个数的位置，$\dots \dots $，第 $k$ 段的第一个数的位置（记作 $q$），那么这个划分方案也就确定了。

考虑到 $q-p\le dist$，本题相当于在一个大小固定为 $dist+1$ 的滑动窗口内，求前 $k-1$ 小的元素和。
...
[视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1oV411D7gB%2F) 第四题。
...
- 时间复杂度：$O(n\log dist)$，其中 $n$ 为 $nums$ 的长度。
- 空间复杂度：$O(dist)$。
