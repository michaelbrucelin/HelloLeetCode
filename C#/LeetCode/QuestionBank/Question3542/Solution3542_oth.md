### [从分治到单调栈，简洁写法（Python/Java/C++/Go）](https://leetcode.cn/problems/minimum-operations-to-convert-all-elements-to-zero/solutions/3673804/cong-fen-zhi-dao-dan-diao-zhan-jian-ji-x-mzbl/)
...
1. 先通过一次操作，把 $nums$ 的最小值都变成 $0$（如果最小值已经是 $0$ 则跳过这步）。
2. 此时 $nums$ 被这些 $0$ 划分成了若干段，后续操作只能在每段内部，不能跨段操作（否则子数组会包含 $0$）。每一段是规模更小的子问题，可以用第一步的方法解决。这样我们可以写一个递归去处理。递归边界：如果操作后全为 $0$，直接返回。
...
具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1m7EuzqEqr%2F%3Ft%3D2m47s)，欢迎点赞关注~
...
- 时间复杂度：$O(n)$，其中 $n$ 是 $nums$ 的长度。每个元素至多入栈出栈各一次，所以二重循环的循环次数是 $O(n)$。
- 空间复杂度：$O(n)$ 或 $O(1)$。原地做法可以做到 $O(1)$ 空间。
