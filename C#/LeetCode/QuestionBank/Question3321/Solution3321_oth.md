### [两个有序集合维护前 x 大二元组（Python/Java/C++/Go）](https://leetcode.cn/problems/find-x-sum-of-all-k-long-subarrays-ii/solutions/2948867/liang-ge-you-xu-ji-he-wei-hu-qian-x-da-p-2rcz/)
...
本题要维护前 $x$ 大的二元组 $(cnt[x],x)$，以及 $cnt[x]\cdot x$ 的总和。其中 $cnt[x]$ 表示 $x$ 在子数组（滑动窗口）中的出现次数。
...
添加删除的同时维护 $cnt[x]\cdot x$ 的总和。

**其余逻辑同 $3013$ 题**。

[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1zU2zYiEa4%2F%3Ft%3D32m55s)，欢迎点赞关注~
...
- 时间复杂度：$O(n\log k)$，其中 $n$ 是 $nums$ 的长度。
- 空间复杂度：$O(n)$。
