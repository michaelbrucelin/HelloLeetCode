...
namespace LeetCode.QuestionBank.Question0306
{
    public class Solution0306 : Interface0306
    {
        ...
        /// <summary>
        /// 逻辑同IsAdditiveNumber()，由Int128改为long
        /// 由于题目限定num不会超过35位，而至少需要3个数字，那么可能最大的数字为17位，17+1+17=35，所以long就够用
        /// </summary>
        /// <param name="num"></param>
        /// <returns></returns>
        public bool IsAdditiveNumber(string num)
        {
            int len = num.Length, _len, LEN = (num.Length - 1) >> 1;
            long n1 = 0, n2, _n1, _n2, _n3; string _n3str;
            for (int i = 0; i < LEN; i++)
            {
                if (i > 0 && num[0] == '0') break;              // 没有含前导0的数字
                n1 = n1 * 10 + num[i] - '0'; n2 = 0;
                for (int j = i + 1, p; j - i <= LEN; j++)
                {
                    if (j > i + 1 && num[i + 1] == '0') break;  // 没有含前导0的数字
                    n2 = n2 * 10 + num[j] - '0';
                    _n3 = n1 + n2;
