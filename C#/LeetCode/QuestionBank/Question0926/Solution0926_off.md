### [将字符串翻转到单调递增](https://leetcode.cn/problems/flip-string-to-monotone-increasing/solutions/1592230/jiang-zi-fu-chuan-fan-zhuan-dao-dan-diao-stjd/?envType=problem-list-v2&envId=h15Gc5qm)
...
当 $i=0$ 时，对应长度为 $1$ 的前缀，一定满足单调递增，因此 $dp[0][0]$ 和 $dp[0][1]$ 的值取决于字符 $s[i]$。具体而言，$dp[0][0]=I(s[0]='1')$，$dp[0][1]=I(s[0]='0')$，其中 $I$ 为示性函数，当事件成立时示性函数值为 $1$，当事件不成立时示性函数值为 $0$。
...
在计算 $dp[i][0]$ 和 $dp[i][1]$ 时，还需要根据 $s[i]$ 的值决定下标 $i$ 处的字符是否需要翻转，因此可以得到如下状态转移方程，其中 $I$ 为示性函数：

$$\begin{array}{l}dp[i][0]=dp[i-1][0]+I(s[i]='1')\\ dp[i][1]=min(dp[i-1][0],dp[i-1][1])+I(s[i]='0')\end{array}$$
