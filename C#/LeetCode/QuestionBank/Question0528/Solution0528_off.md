### [按权重随机选择](https://leetcode.cn/problems/random-pick-with-weight/solutions/966239/an-quan-zhong-sui-ji-xuan-ze-by-leetcode-h13t/?envType=problem-list-v2&envId=ySsxoJfz)
...
$$pre[i]=\sum\limits_{k=0}^i w[k]$$

那么第 $i$ 个区间的左边界就是 $pre[i]-w[i]+1$，右边界就是 $pre[i]$。

当划分完成后，假设我们随机到了整数 $x$，我们希望找到满足：

$$pre[i]-w[i]+1\le x\le pre[i]$$

的 $i$ 并将其作为答案返回。由于 $pre[i]$ 是单调递增的，因此我们可以使用二分查找在 $O(\log n)$ 的时间内快速找到 $i$，即找出最小的满足 $x\le pre[i]$ 的下标 $i$。
