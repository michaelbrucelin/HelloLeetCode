### [【图解】DP 及其优化：从 $n^4$ 到 $n^3$ 到 $n^2$（Python/Java/C++/Go）](https://leetcode.cn/problems/maximum-score-from-grid-operations/solutions/2852362/tu-jie-dp-ji-qi-you-hua-by-endlesscheng-pco6/)

#### 一、从超时做法说起

从 $O(n^4)$ 的 $DP$ 开始。

要想知道第 $j$ 列的哪些行的 $grid[i][j]$ 被计入总分，我们需要知道：

- 第 $j+1$ 列有多少个黑色格子（下文简称为黑格）。
- 第 $j$ 列有多少个黑格。
- 第 $j-1$ 列有多少个黑格。

定义 $dfs(j,cur,pre)$ 表示考虑第 $0$ 列到第 $j$ 列，其中第 $j+1$ 列有 $pre$ 个黑格、第 $j$ 列有 $cur$ 个黑格，返回在第 $0$ 列到第 $j$ 列中能得到的最大总分。

枚举第 $j-1$ 列有 $nxt$ 个黑格，问题变成：

- 考虑第 $0$ 列到第 $j-1$ 列，其中第 $j$ 列有 $cur$ 个黑格，第 $j-1$ 列有 $nxt$ 个黑格，在第 $0$ 列到第 $j-1$ 列中能得到最大总分，即 $dfs(j-1,nxt,cur)$。

定义 $s$ 为 $grid[cur][j]$ 到 $grid[max(nxt,pre)-1][j]$ 的元素和，如果 $max(nxt,pre)\le cur$ 则 $s=0$。

用 $dfs(j-1,nxt,cur)+s$ 更新 $dfs(j,cur,pre)$ 的最大值。

递归边界：$j=0$ 时返回 $s$。

递归入口：$\mathop{max}\limits_{i=0}^{n}dfs(n-1,i,0)$。枚举第 $n-1$ 列有 $i$ 个黑格，取递归结果的最大值，作为答案。
...
- 时间复杂度：$O(n^4)$，其中 $n$ 是 $grid$ 的长度。由于每个状态只会计算一次，动态规划的时间复杂度 $=$ 状态个数 $\times$ 单个状态的计算时间。本题状态个数等于 $O(n^3)$，单个状态的计算时间为 $O(n)$，所以动态规划的时间复杂度为 $O(n^4)$。
- 空间复杂度：$O(n^3)$。

#### 二、记忆化搜索

如果不考虑第 $j-1$ 列呢？不去枚举 $nxt$，而是枚举 $cur$ 呢？

我们的原则是，在从右往左递归的过程中，只把第 $j$ 列或者第 $j+1$ 列的格子计入总分，不考虑第 $j-1$ 列的格子。

如何**不重不漏**地统计呢？

![](./assets/img/Solution3225_oth.png)

定义 $dfs(j,pre,dec)$ 表示考虑第 $0$ 列到第 $j$ 列，其中：

- 第 $j+1$ 列有 $pre$ 个黑格；
- 第 $j+1$ 列和第 $j+2$ 列的黑格个数的大小关系用布尔值 $dec$ 表示，只有当第 $j+1$ 列的黑格个数小于第 $j+2$ 列的黑格个数时 $dec$ 才为 $true$。

在上述约束下，返回第 $0$ 列到第 $j$ 列中能得到的最大总分。

枚举第 $j$ 列有 $cur$ 个黑格，按照上图中的四种情况计算。

递归边界：$j=-1$ 时返回 $0$。

递归入口：$\mathop{max}\limits_{i=0}^{n}dfs(n-2,i,0)$。枚举第 $n-1$ 列有 $i$ 个黑格，取递归结果的最大值，作为答案。注意第 $n-1$ 列的格子会在 $j=n-2$ 中计入。
...
- 时间复杂度：$O(n^3)$，其中 $n$ 是 $grid$ 的长度。由于每个状态只会计算一次，动态规划的时间复杂度 $=$ 状态个数 $\times $ 单个状态的计算时间。本题状态个数等于 $O(n^2)$，单个状态的计算时间为 $O(n)$，所以动态规划的时间复杂度为 $O(n^3)$。
- 空间复杂度：$O(n^2)$。
...
f[j+1][pre][dec] 的定义和 $dfs(j,pre,dec)$ 的定义是一样的。注意这里是 $j+1$ 不是 $j$，因为要避免 $j=-1$ 时出现负数下标。

初始值 $f[0][pre][dec]=0$，翻译自递归边界。

答案为 $\mathop{max}\limits_{i=0}^{n}f[n-1][i][0]$，翻译自递归入口。
...
首先计算 $pre>0$ 的状态，然后单独计算 $pre=0$ 的状态。

##### 1) #pre > 0$ 且 $dec = 1$

$pre>0$ 的状态，没有情况四。

对于 $f[j+1][pre][1]$，需要计算 $f[j][pre][0]$（情况一）与下式（情况二）的最大值：

$$\begin{matrix}
      & \mathop{max}\limits_{cur=0}^{pre-1}\{f[j][cur][1]+colSum[j][pre]-colSum[j][cur]\} \\
    = & colSum[j][pre]+\mathop{max}\limits_{cur=0}^{pre-1}\{f[j][cur][1]-colSum[j][cur]\}
\end{matrix}$$

其中

$$\mathop{max}\limits_{cur=0}^{pre-1}\{f[j][cur][1]-colSum[j][cur]\}$$

可以一边**从小到大**枚举 $pre$，一边用一个变量 $preMax$ 维护。

##### 2) $pre > 0$ 且 $dec = 0$

对于 $f[j+1][pre][0]$，除了上面 $dec=1$ 要计算的，这里也要计算外，还需要计算下式（情况三）的最大值：

$$\begin{cases}
      & \mathop{max}\limits_{cur=pre+1}^{n}\{f[j][cur][0]+colSum[j+1][cur]-colSum[j+1][pre]\} \\
    = & -colSum[j+1][pre]+\mathop{max}\limits_{cur=pre+1}^{n}\{f[j][cur][0]+colSum[j+1][cur]\}
\end{cases}$$

其中

$$\mathop{max}\limits_{cur=pre+1}^{n}\{f[j][cur][0]+colSum[j+1][cur]\}$$

可以一边**从大到小**枚举 $pre$，一边用一个变量 $sufMax$ 维护。
...
$$\mathop{max}\limits_{cur=1}^{n}\{f[j][cur][0]+colSum[j+1][cur]\}$$
...
$$\mathop{max}\limits_{cur=0}^{n}f[j][cur][0]$$

但在 $pre=0$ 且 $dec=1$ 的前提下，其实只需考虑第 $j$ 列全白（$cur=0$）或全黑（$cur=n$）两种情况。沿用上文图片中的证明方法，考虑第 $j-1$ 列的黑格个数 $B_{j-1}$：
