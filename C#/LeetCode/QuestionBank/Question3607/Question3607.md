...
这些电站通过 `n` 条 **双向** 电缆互相连接，表示为一个二维数组 `connections`，其中每个元素 `connections[i] = [u<sub>i</sub>, v<sub>i</sub>]` 表示电站 <code>u<sub>i</sup></code> 和电站 <code>v<sub>i</sup></code> 之间的连接。直接或间接连接的电站组成了一个 **电网**。
...
- <code>0 <= n == connections.length <= min(10<sup>5</sup>, c &times; (c - 1) / 2)</code>
- `connections[i].length == 2`
- <code>1 <= u<sub>i</sub>, v<sub>i</sub> <= c</code>
- <code>u<sub>i</sub> != v<sub>i</sup></code>
- <code>1 <= queries.length <= 2 &times; 10<sup>5</sup></code>
- `queries[i].length == 2`
- `queries[i][0]` 为 `1` 或 `2`。
- `1 <= queries[i][1] <= c`
