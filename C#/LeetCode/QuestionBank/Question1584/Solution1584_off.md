### [连接所有点的最小费用](https://leetcode.cn/problems/min-cost-to-connect-all-points/solutions/565801/lian-jie-suo-you-dian-de-zui-x_iao-fei-yo-kcx7/)
...
int minCostConnectPoints(int** points, int pointsSize, int* pointsColSize) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            edges[edgesSize].x = i;
            edges[edgesSize].y = j;
            edges[edgesSize++].len = fabs(points[i][0] - points[j][0]) +
...
- 时间复杂度：$O(n^2\log (n))$，其中 $n$ 是节点数。一般 $Kruskal$ 是 $O(m\log m)$ 的算法，但本题中 $m=n2$，因此总时间复杂度为 $O(n^2\log (n))$。
...
方法一中，虽然使用了 $Kruskal$ 算法，但时间复杂度仍然较高，因为本题中的边数是 $O(n^2)$ 的，所以我们需要想办法将减少边数。为此，我们提出几个结论：
...
![](./assets/img/Solution1584_off_2.png)

图中任意一个区块的两分割线的夹角均为 $45\circ $。

我们以 $P1$ 区块为例，假设 $B(x_B,y_B),C(x_C,y_C)$，不失一般性，假设 $x_B+y_B\le x_C+y_C$。

因为处于 $P1$ 区域，所以有 $0\le x_B\le y_B$，0\le $x_C\le y_C$。所以 $BC=\vert x_B-x_C\vert +\vert y_B-y_C\vert $。

下面我们尝试分类讨论：

1. 当 $x_B>x_C,y_B>y_C$，这与 $x_B+y_B\le x_C+y_C$ 矛盾。
2. 当 $x_B\le x_C,y_B>y_C$，此时有 $\vert BC\vert =x_C-x_B+y_B-y_C$，\vert $AC\vert -\vert BC\vert =x_C+y_C-x_C+x_B-y_B+y_C=x_B-y_B+2\times y_C$。由前面各种关系可得 $y_B>y_C>x_C>x_B$。假设 $\vert AC\vert <\vert BC\vert $，即 $y_B>2\times y_C+x_B$，那么 $\vert AB\vert =x_B+y_B>2\times x_B+2\times y_C$，$\vert AC\vert =x_C+y_C<2\times y_C<\vert AB\vert $ 与前提矛盾，故 $\vert AC\vert \ge \vert BC\vert $；
3. $x_B>x_C$ 且 $y_B\le y_C$。与 $2$ 同理；
4. $x_B\le x_C$ 且 $y_B\le y_C$。此时显然有 $\vert AB\vert +\vert BC\vert =\vert AC\vert $，即有 $\vert AC\vert >\vert BC\vert $。

综上有 $\vert AC\vert \ge \vert BC\vert $，这个性质可以从 $P1$ 区域推导到其他七个区域。
...
下面代码中的 $Python$ 代码中需要 $import$ 类型标注中的 $Tuple$，当然删去对应部分也可以成功运行。
...
- 时间复杂度：$O(n\log n)$，其中 $n$ 是节点数。预处理建边的时间复杂度为 $O(n\log n)$，因为需要排序，以及使用树状数组维护。在只有 $O(n)$ 条边的情况下，$Kruskal$ 的时间复杂度为 $O(n\log n)$，因此总时间复杂度为 $O(n\log n)$。
