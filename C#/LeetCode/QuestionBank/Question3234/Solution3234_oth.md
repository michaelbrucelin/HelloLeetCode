### [枚举子串中的 0 的个数（Python/Java/C++/C/Go/JS/Rust）](https://leetcode.cn/problems/count-the-number-of-substrings-with-dominant-ones/solutions/2860198/mei-ju-zi-chuan-zhong-de-0-de-ge-shu-pyt-c654/)

#### 分析

设 $cnt_0$ 为子串中的 $0$ 的个数，$cnt_1$ 为子串中的 $1$ 的个数。

根据题意，$1$ 显著子串必须满足

$$0\le cnt_0^2\le cnt_1\le n$$

解得

$$0\le cnt_0\le \sqrt{n}$$
...
那么右端点**固定**为 $r$ 的子串 $[i+1,r],[i+2,r],\dots ,[r,r]$ 都是不包含 $0$ 的，一定是 $1$ 显著子串，这有 $r-i$ 个。

设上上一个 $0$ 的下标为 $j$。

那么子串 $[j+1,r],[j+2,r],\dots ,[i,r]$ 都恰好有 $1$ 个 $0$。
...
一般地，外层循环枚举子串右端点 $r=0,1,2,\dots ,n-1$，内层循环枚举子串**恰好**有 $cnt_0=0,1,2,\dots $ 个 $0$，如果 $cnt_0^2$ 超过了 $[0,r]$ 中的 $1$ 的个数，则跳出枚举 $cnt_0$ 的循环。

设当前枚举的 $cnt_0$ 对应的左右两个 $0$ 的下标分别为 $p$ 和 $q$。

右端点为 $r$ 且恰好有 $cnt_0$ 个 $0$ 的最短子串为 $[q,r]$，设其有 $cnt_1$ 个 $1$。

分类讨论：

- 如果 $cnt_0^2\le cnt_1$，那么子串左端点可以是 $p+1,p+2,\dots ,q$，一共 $q-p$ 个合法左端点。
- 如果 $cnt_0^2>cnt_1$，那么还需要包含至少 $cnt_0^2-cnt_1$ 个 $1$，所以子串左端点的最大值为 $q-(cnt_0^2-cnt_1)$，最小值为 $p+1$，一共 $q-(cnt_0^2-cnt_1)-p$ 个合法左端点。如果个数是负数，则没有合法左端点。

综合一下，合法左端点最小是 $p+1$，最大是 $q-max(cnt_0^2-cnt_1,0)$，所以一共有

$$q-max(cnt_0^2-cnt_1,0)-p$$

个合法左端点。考虑到上式可能是负数，所以还要再与 $0$ 取最大值，得

$$max(q-max(cnt_0^2-cnt_1,0)-p,0)$$
...
- 时间复杂度：$O(n\sqrt{n})$，其中 $n$ 是 $s$ 的长度。
- 空间复杂度：$O(n)$。

**注**：使用队列，只保存 $r$ 及其左侧的 $O(\sqrt{n})$ 个 $0$ 的下标，可以把空间复杂度优化到 $O(\sqrt{n})$。
