### [三种方法：按照 core 分组 / 枚举 core / 打表预处理（Python/Java/C++/Go/JS）](https://leetcode.cn/problems/maximum-element-sum-of-a-complete-subset-of-indices/solutions/2446037/an-zhao-corei-fen-zu-pythonjavacgo-by-en-i6nu/?envType=problem-list-v2&envId=ySsxoJfz)

#### 方法一：按照下标的 core 值分组

定义 $core(n)$ 为 $n$ 除去完全平方因子后的剩余结果。

例如 $core(8)=8/4=2, core(12)=12/4=3,core(25)=25/25=1,core(5)=5/1=5$。
...
[视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1Lm4y1N7mf%2F) 第四题。
...
- 时间复杂度：$O(n\sqrt{n})$，其中 $n$ 为 $nums$ 的长度。
- 空间复杂度：$O(n)$。

#### 方法二：枚举 core

我们还可以从 $1$ 开始枚举 $i$ 和所有 $core$ 值等于 $i$ 的下标，也就是 $ij^2$。

例如 $i=3$ 时，我们可以枚举

$$3\times 1,3\times 4,3\times 9,3\times 16,3\times 25,\dots$$

这些下标对应的元素值都在同一组中。

可能你会觉得这不对，比如 $i=4$ 时，我们枚举的是

$$4\times 1,4\times 4,4\times 9,4\times 16,4\times 25,\dots$$

而这些数的 $core$ 值都等于 $1$，而不是 $4$。
...
- 时间复杂度：$O(n)$，其中 $n$ 为 $nums$ 的长度。循环次数可近似为 $\sqrt{n}\cdot (1/\sqrt{1}+1/\sqrt{2}+...+1/\sqrt{n})$，由 $f(x)=1/\sqrt{x}$ 的积分可知，$1/\sqrt{1}+1/\sqrt{2}+...1/\sqrt{n}=O(\sqrt{n})$，所以总的循环次数为 $O(n)$。
- 空间复杂度：$O(1)$。

#### 方法三：预处理所有 core 值

按照方法二的计算方式，可以直接打表预处理 $10^4$ 内的 $core$ 值。
