### [第 K 个最小的素数分数](https://leetcode.cn/problems/k-th-smallest-prime-fraction/solutions/1127103/di-k-ge-zui-xiao-de-su-shu-fen-shu-by-le-argw/?envType=problem-list-v2&envId=ySsxoJfz)
...
当分母为给定的 $arr[j]$ 时，分子可以在 $arr[0],\dots ,arr[j-1]$ 中进行选择。由于数组 $arr$ 是严格递增的，那么记分子为 $arr[i] (0\le i<j)$，随着 $i$ 的增加，分数的值也是严格递增的。

因此我们可以将每个分母 $arr[j]$ 看成一个长度为 $j$ 的列表，它包含了：

$$\dfrac{arr[0]}{arr[j]},\dfrac{arr[1]}{arr[j]},\dots ,\dfrac{arr[j-1]}{arr[j]}$$

这些分数，并且它们的值是严格递增的。我们的目标是找出这 $n-1$ 个列表（$arr[0]$ 的列表为空，我们可以直接忽略）中第 $k$ 小的素数分数，这就提示我们参考[「23. 合并 $K$ 个升序链表」](https://leetcode.cn/problems/merge-k-sorted-lists/)中的方法，使用优先队列来得到答案。

初始时，优先队列中存储了 $n-1$ 个分数 $\dfrac{arr[0]}{arr[1]},\dots ,\dfrac{arr[0]}{arr[n-1]}$。在求解答案的过程中，我们每次从优先队列中取出一个最小的分数，记为 $\dfrac{arr[i]}{arr[j]}$。如果 $i+1<j$，我们将一个新的分数 $\dfrac{arr[i+1]}{arr[j]}$ 放入优先队列中。这样一来，当我们进行第 $k$ 次「取出」操作时，得到的分数就是第 $k$ 小的素数分数。
...
- 我们再使用一个指针 $i$ 指向分子，这个指针会不断向右移动，并且保证 $\dfrac{arr[i]}{arr[j]}<\alpha $ 一直成立。当指针 $i$ 无法移动时，我们就可以知道 $arr[0],\dots ,arr[i]$ 都可以作为分子，但 $arr[i+1]$ 及以后的元素都不可以，即分母为 $arr[j]$ 并且小于 $\alpha$ 的素数分数有 $i+1$ 个。
- 在 $j$ 向右移动的过程中，我们把每一个 $j$ 对应的 $i+1$ 都加入答案。这样在双指针的过程完成后，我们就可以得到有多少个小于 $\alpha$ 的素数分数了。

如果我们得到的答案恰好等于 $k$，那么我们再进行一遍上面的过程，求出所有 $\dfrac{arr[i]}{arr[j]}$ 中的最大值即为第 $k$ 个最小的素数分数。但如果答案小于 $k$，这说明我们猜测的 $\alpha$ 太小了，我们需要增加它的值；如果答案大于 $k$，这说明我们猜测的 $\alpha$ 太大了，我们需要减少它的值。
...
- 时间复杂度：$O(n\log C)$，其中 $n$ 是数组 $arr$ 的长度，$C$ 是数组 $arr$ 中元素的上界。二分查找需要进行 $\lceil \log C^2\rceil =O(\log C)$ 次，每一步需要 $O(n)$ 的时间得到小于 $\alpha$ 的素数分数的个数。
- 空间复杂度：$O(1)$。
