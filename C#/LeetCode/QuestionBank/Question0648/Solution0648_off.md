### [单词替换](https://leetcode.cn/problems/replace-words/solutions/1649109/dan-ci-ti-huan-by-leetcode-solution-pl6v/)

#### 方法一：哈希集合

**思路**

首先将 $dictionary$ 中所有词根放入哈希集合中，然后对于 $sentence$ 中的每个单词，由短至长遍历它所有的前缀，如果这个前缀出现在哈希集合中，则我们找到了当前单词的最短词根，将这个词根替换原来的单词。最后返回重新拼接的句子。
...
- 时间复杂度：$O(d+\sum w_i^2)$。其中 $d$ 是 $dictionary$ 的字符数，构建哈希集合消耗 $O(d)$ 时间。$wi$ 是 $sentence$ 分割后第 $i$ 个单词的字符数，判断单词的前缀子字符串是否位于哈希集合中消耗 $O(w_i^2)$ 时间。
