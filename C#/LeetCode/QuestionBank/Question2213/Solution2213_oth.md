### [从分治出发思考，附相似题目（Python/Java/C++/Go）](https://leetcode.cn/problems/longest-substring-of-one-repeating-character/solutions/1352584/by-endlesscheng-qpbw/)
...
- $mx$：该区间的最长连续相同子串的长度。这等于上面分治过程的三种情况的最大值。
- $pre$：该区间最长连续相同前缀的长度。计算方法见下。
- $suf$：该区间最长连续相同后缀的长度。计算方法见下。

我们合并区间 $A$ 和 $B$，设合并后的大区间为 $C$。

- 如果 $A.pre$ 等于 $A$ 的长度，且 $A$ 的最后一个字符等于 $B$ 的第一个字符，那么 $C.pre=A.pre+B.pre$。否则，$C.pre=A.pre$。
- 如果 $B.suf$ 等于 $B$ 的长度，且 $A$ 的最后一个字符等于 $B$ 的第一个字符，那么 $C.suf=A.suf+B.suf$。否则，$C.suf=B.suf$。
...
- 时间复杂度：$O(n+q\log n)$，其中 $n$ 是 $s$ 的长度，$q$ 是 $queryCharacters$ 的长度。
- 空间复杂度：$O(n)$。返回值不计入。
