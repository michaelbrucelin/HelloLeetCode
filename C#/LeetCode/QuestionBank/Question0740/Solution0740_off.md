### [删除并获得点数](https://leetcode.cn/problems/delete-and-earn/solutions/758416/shan-chu-bing-huo-de-dian-shu-by-leetcod-x1pu/)
...
记元素 $x$ 在数组中出现的次数为 $cx$，我们可以用一个数组 $sum$ 记录数组 $nums$ 中所有相同元素之和，即 $sum[x]=x\cdot cx$。若选择了 $x$，则可以获取 $sum[x]$ 的点数，且无法再选择 $x-1$ 和 $x+1$。这与「[198\. 打家劫舍](https://leetcode.cn/problems/house-robber/)」是一样的，在统计出 $sum$ 数组后，读者可参考「[198\. 打家劫舍的官方题解](https://leetcode.cn/problems/house-robber/solution/da-jia-jie-she-by-leetcode-solution/)」中的动态规划过程计算出答案。
...
- 时间复杂度：$O(N\log N)$，其中 $N$ 是数组 $nums$ 的长度。对 $nums$ 排序需要花费 $O(N\log N)$ 的时间，遍历计算需要花费 $O(N)$ 的时间，故总的时间复杂度为 $O(N\log N)$。
