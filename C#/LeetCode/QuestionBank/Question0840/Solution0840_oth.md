### [无需计算第三行列的和，以及对角线的和，数学证明（Python/Java/C++/Go）](https://leetcode.cn/problems/magic-squares-in-grid/solutions/3867033/wu-xu-ji-suan-di-san-xing-lie-de-he-yi-j-djue/)

#### 幻方正中心一定是 5

$3\times 3$ 的幻方必须包含 $1$ 到 $9$ 所有数字，这意味着幻方元素和为 $1+2+\dots +9=45$。
...
如何快速判断矩阵包含 $1$ 到 $9$ 所有数？可以把数字压缩到一个二进制数 $mask$ 中，$mask$ 从低到高的 $i$ 位是 $1$ 表示 $i$ 在矩阵中。矩阵包含 $1$ 到 $9$ 所有数相当于 $mask=1111111110_{(2)}=2^{10}-2=1022$。
