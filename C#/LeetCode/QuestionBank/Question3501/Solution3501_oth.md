### [区间最大值 + 分类讨论（Python/Java/C++/Go）](https://leetcode.cn/problems/maximize-active-section-with-trade-ii/solutions/3633400/qu-jian-zui-da-zhi-er-fen-cha-zhao-fen-l-n832/)

**前置题目**：[3499\. 操作后最大活跃区段数 I](https://leetcode.cn/problems/maximize-active-section-with-trade-i/)。

设 $s$ 中的 $1$ 的个数为 $total1$。

设 $s$ 中的所有连续 $0$ 对应的（左闭右开）区间列表为 $a$。例如 $s=0100$ 中有两段连续 $0$，区间分别为 $[0,1)$ 和 $[2,4)$。

在前置题目中，我们知道，答案为 $total1$ 加上 $0$ 最多的 $010$ 子串中的 $0$ 的个数 $mx$。

对于询问 $[ql,qr)$（改成右开），分类讨论：

- 如果 $[ql,qr)$ 中没有完整的区间，但包含一段完整的 $1$，那么 $mx$ 为两个残缺的区间长度之和。
- 如果 $[ql,qr)$ 中有完整的区间，那么 $mx$ 为以下三种情况的最大值：
  - $[ql,qr)$ 中的相邻完整区间的长度之和的最大值。这可以用线段树或者 $ST$ 表统计。线段树的模板见 [数据结构题单](https://leetcode.cn/circle/discuss/mOr1u6/)。
  - $ql$ 所处的残缺区间与 $[ql,qr)$ 的第一个完整区间的长度之和。
  - $qr$ 所处的残缺区间与 $[ql,qr)$ 的最后一个完整区间的长度之和。

计算 $[ql,qr)$ 中的第一个完整区间和最后一个完整区间，可以用二分查找。

对于最后一个完整区间，可以先二分找到右端点 $>qr$ 的第一个区间，这个区间的左边相邻区间，就是最后一个完整区间。

代码实现时，可以用哨兵简化代码，无需判断下标是否在边界上。可以把计算两个区间长度之和的逻辑，封装成一个函数。

具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1JrZzYhEHt%2F%3Ft%3D6m9s)，欢迎点赞关注\sim
...
- 时间复杂度：$O((n+q)\log n)$，其中 $n$ 是 $s$ 的长度。
- 空间复杂度：$O(n\log n)$。
...
- 时间复杂度：$O(n\log n+q)$，其中 $n$ 是 $s$ 的长度。预处理后，可以做到 $O(1)$ 回答每个询问！
- 空间复杂度：$O(n\log n)$。
