### [网格图 DP + 后缀最小值优化 + 收敛优化（Python/Java/C++/Go）](https://leetcode.cn/problems/minimum-cost-path-with-teleportations/solutions/3755166/wang-ge-tu-dp-hou-zhui-zui-xiao-zhi-you-b014t/)
...
- 传送：设 $x=grid[i][j]$，我们可以从格子值 $\ge x$ 的任意格子传送到 $(i,j)$。转移来源为 $f[t-1][i^′+1][j^′+1]$，满足 $grid[i^′][j^′]\ge x$。如何快速得到这些 $f[t-1][i^′+1][j^′+1]$ 的最小值？
  - 定义 $sufMinF_{t-1}[x]$ 表示满足 $grid[i][j]\ge x$ 的 $f[t-1][i+1][j+1]$ 的最小值。
  - 在计算完 $f[t-1][i+1][j+1]$ 后，把格子值 $x=grid[i][j]$ 及其对应的状态值 $f[t-1][i+1][j+1]$ 保存到一个数组 $minF$ 中，其中 $minF[x]$ 表示格子值为 $x$ 的最小状态值（如果不存在则为 $\infty $）。然后倒序遍历 $minF$，计算后缀最小值，即为 $sufMinF_{t-1}$。

状态转移方程为

$$f[t][i+1][j+1]=min(f[t][i+1][j]+x,f[t][i][j+1]+x,sufMinF_{t-1}[x])$$
...
具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV191YCzjEvc%2F%3Ft%3D15m04s)，欢迎点赞关注~
...
每次循环我们会计算一遍 $sufMinF$。如果发现某次循环没有改变 $sufMinF$，那么无论再传送多少次，都不会再改变 $sufMinF$ 了，此时我们已经找到了答案。

力扣喜欢出随机数据。测试发现，对于 $m=n=80$，值域在 $[0,10^4]$ 中随机的测试数据，平均迭代约 $2.2$ 次就收敛了，然后再循环一次发现收敛，即 $sufMinF$ 在循环前后是相同的。所以平均外层循环约 $3.2$ 次就可以退出循环了，而不是循环 $k+1$ 次。

此外，如果 $k>0$ 且可以直接跳到终点，即 $grid[0][0]\ge grid[m-1][n-1]$，那么直接返回 $0$。
...
- 时间复杂度：$O((mn+U)k)$，其中 $m$ 和 $n$ 分别为 $grid$ 的行数和列数，$U$ 为 $grid[i][j]$ 的最大值。
- 空间复杂度：$O(n+U)$。
