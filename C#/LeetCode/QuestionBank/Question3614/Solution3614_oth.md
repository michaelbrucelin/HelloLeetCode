### [逆向思维（Python/Java/C++/Go）](https://leetcode.cn/problems/process-string-with-special-operations-ii/solutions/3722462/ni-xiang-si-wei-pythonjavacgo-by-endless-26al/)

推荐先把本题的简单版本 [3307\. 找出第 K 个字符 II](https://leetcode.cn/problems/find-the-k-th-character-in-string-game-ii/) 做了。

正着做可以吗？

生成的 $result$ 字符串太长，超出内存限制，不行。

那么，倒着做可以吗？

为了方便下面计算 $k$ 的位置，首先，计算出每个 $s[i]$ 处理完后的 $result$ 的长度，记作 $size[i]$。最终 $result$ 的长度为 $size[n-1]$。

首先特判 $k\ge size[n-1]$ 的情况，下标越界，返回 $.$ 号。

否则答案一定存在。我们从 $c=s[n-1]$ 开始倒着思考：

- 如果 $c$ 是字母：
  - 如果 $k=size[n-1]-1$，那么答案就是 $result$ 的最后一个字母，即 $c$。
  - 否则需要继续分析，问题变成 $s$ 的前 $n-1$ 个字母的**子问题**，可以**递归/迭代**解决。
- 如果 $c=*$：
  - 无影响（我们已经特判下标越界的情况），问题变成 $s$ 的前 $n-1$ 个字母的子问题。
- 如果 $c=\#$：
  - 设 $m=\dfrac{size[n-1]}{2}$。
  - 如果 $k<m$，无影响，问题变成 $s$ 的前 $n-1$ 个字母的子问题。
  - 如果 $k\ge m$，问题变成 $s$ 的前 $n-1$ 个字母的子问题中，下标为 $k-m$ 的字母（这个字母复制后的下标就是 $k$）。
- 如果 $c=%$：
  - 问题变成 $s$ 的前 $n-1$ 个字母的子问题中，下标为 $size[n-1]-1-k$ 的字母（这个字母反转后的下标就是 $k$）。

其余步骤和最后一步的处理逻辑是完全一样的，所以**想清楚最后一步怎么做，就想清楚了每一步怎么做**。

下面代码用迭代解决。

具体请看 [视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1xSuFzHEa1%2F%3Ft%3D6m41s)，欢迎点赞关注$\sim$
...
$size$ 数组可以去掉，只需要 $sz$ 变量。在第二个循环中，我们可以倒推还原 $sz$。

你可能会问：`sz = max(sz - 1, 0)` 这一步怎么倒推？

事实上，倒推的过程中不会出现 $sz=0$ 的情况。如果出现，说明我们把前面的字母全部删掉了，那么前面不可能找到答案，矛盾。换句话说，我们要么返回 $.$ 号，要么一定会在 $sz=0$ 之前就找到答案。
