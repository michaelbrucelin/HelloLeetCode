### [去除重复字母](https://leetcode.cn/problems/remove-duplicate-letters/solutions/527359/qu-chu-zhong-fu-zi-mu-by-leetcode-soluti-vuso/?envType=problem-list-v2&envId=ySsxoJfz)
...
- 空间复杂度：$O(\vert \sum \vert)$，其中 $\sum $ 为字符集合，本题中字符均为小写字母，所以 $\vert \sum \vert =26$。由于栈中的字符不能重复，因此栈中最多只能有 $\vert \sum \vert$ 个字符，另外需要维护两个数组，分别记录每个字符是否出现在栈中以及每个字符的剩余数量。
