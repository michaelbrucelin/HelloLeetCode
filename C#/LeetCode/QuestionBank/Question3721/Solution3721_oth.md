### [两种方法维护前缀和：Lazy 线段树 / 分块（Python/Java/C++/Go）](https://leetcode.cn/problems/longest-balanced-subarray-ii/solutions/3809924/fen-kuai-wei-hu-qian-zhui-he-pythonjavac-rt79/)
...
3. [Lazy 线段树](https://leetcode.cn/link/?target=https%3A%2F%2Foi-wiki.org%2Fds%2Fseg%2F%23%E7%BA%BF%E6%AE%B5%E6%A0%91%E7%9A%84%E5%8C%BA%E9%97%B4%E4%BF%AE%E6%94%B9%E4%B8%8E%E6%87%92%E6%83%B0%E6%A0%87%E8%AE%B0)。
...
从左到右遍历 $nums$，如果固定子数组右端点为 $i$，要想让子数组包含某个元素 $x$，左端点必须 $\le x$ 最后一次出现的位置。只要子数组包含最近遇到的 $x$，那么无论子数组有多长，都包含了 $x$。题目要求，多个 $x$ 只能算一次，那么把**除了最近一次的 $x$ 全部不计入**，就变成 $525$ 题了！

以 $nums=[1,2,1,2,3,3]$ 为例：

- 遍历到 $i=0$，把 $nums$ 视作 $[1,*,*,*,*,*]$。
- 遍历到 $i=1$，把 $nums$ 视作 $[1,2,*,*,*,*]$。
- 遍历到 $i=2$，把 $nums$ 视作 $[*,2,1,*,*,*]$。
- 遍历到 $i=3$，把 $nums$ 视作 $[*,*,1,2,*,*]$。
- 遍历到 $i=4$，把 $nums$ 视作 $[*,*,1,2,3,*]$。
- 遍历到 $i=5$，把 $nums$ 视作 $[*,*,1,2,*,3]$。

根据 $525$ 题，把偶数视作 $-1$，奇数视作 $1$，**遍历过的星号视作** $0$，设这个新数组为 $a$，问题相当于：

- 计算 $a$ 中和为 $0$ 的最长子数组的长度。

设 $a$ 的长为 $n+1$ 的前缀和数组为 $sum$。根据 $525$ 题，问题相当于：

- 枚举 $i$，在 $[0,i-1]$ 中找到一个下标最小的 $sum[j]$，满足 $sum[j]=sum[i]$。
- 用子数组长度 $i-j$ 更新答案的最大值。

根据上面动态变化的过程：

- 设 $x=nums[i]$ 对应的 $a[i]$ 值为 $v$。
- 当我们首次遇到 $x$ 时，对于前缀和 $sum$ 来说，$[i,n]$ 要全部增加 $v$。
- 当我们再次遇到 $x$ 时，原来的 $nums[j]$ 变成星号（$a[j]=0$），$x$ 搬到了新的位置 $i$，所以之前的「[j,n] 全部增加 v」变成了「[i,n] 全部增加 v」，也就是撤销 $[j,i-1]$ 的加 $v$，也就是把 $[j,i-1]$ 减 $v$。
...
[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV18GsAzuE6W%2F%3Ft%3D11m54s)，欢迎点赞关注~

#### 方法一：Lazy线段树

由于 $a$ 中元素只有 $-1,0,1$，所以 $sum$ 数组相邻元素之差 $\le 1$。根据**离散介值定理**，设 $min$ 和 $max$ 分别为区间的最小值和最大值，只要 $sum[i]$ 在 $[min,max]$ 范围中，区间就一定存在等于 $sum[i]$ 的数。
...
见 [分块思想](https://leetcode.cn/link/?target=https%3A%2F%2Foi-wiki.org%2Fds%2Fdecompose%2F)。

这个做法没有用到 $sum$ 数组的特殊性质，支持区间更新、查询任意值首次出现的位置。

每块维护块内 $sum[i]$ 首次出现的位置，以及区间加的 $Lazy tag$。
...
- 时间复杂度：$O(n\sqrt{n})$，其中 $n$ 是 $nums$ 的长度。
- 空间复杂度：$O(n)$。

#### 相似题目

[HH 的项链](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.luogu.com.cn%2Fproblem%2FP1972)
