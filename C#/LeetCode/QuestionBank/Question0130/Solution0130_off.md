### [被围绕的区域](https://leetcode.cn/problems/surrounded-regions/solutions/369110/bei-wei-rao-de-qu-yu-by-leetcode-solution/?envType=problem-list-v2&envId=tBJHVASZ)
...
- 时间复杂度：$O(m\times n)$，其中 $m$ 和 $n$ 分别为矩阵的行数和列数。深度优先搜索过程中，每一个点至多只会被标记一次。
- 空间复杂度：$O(m\times n)$，其中 $m$ 和 $n$ 分别为矩阵的行数和列数。主要为深度优先搜索的栈的开销。
...
- 时间复杂度：$O(m\times n)$，其中 $m$ 和 $n$ 分别为矩阵的行数和列数。广度优先搜索过程中，每一个点至多只会被标记一次。
- 空间复杂度：$O(m+n)$，其中 $m$ 和 $n$ 分别为矩阵的行数和列数。主要为广度优先搜索的队列的开销。由于我们从边界开始扩展，队列中最多只会有 $2\times (m+n)$ 个元素。
