...
{
    public class Solution2065_2 : Interface2065
    {
        /// <summary>
        /// DFS + 回溯
        /// 有返回值
        ...
        public int MaximalPathQuality(int[] values, int[][] edges, int maxTime)
        {
            ...
            int dfs(int position, int point, int maxTime)
            {
                int result = 0;
                ...
                {
                    if (info.time <= maxTime) result = Math.Max(result, dfs(info.next, point, maxTime - info.time));
                }
