### [香槟塔](https://leetcode.cn/problems/champagne-tower/solutions/1979893/xiang-bin-ta-by-leetcode-solution-y87c/)

#### 方法一：模拟

**思路**

可以模拟倒香槟过程。首先将所有的 $poured$ 杯香槟全部倒到 $row=0$ 的这个杯子中。当有溢出时，再将溢出的部分平均倒到下一层的相邻的两个杯子中。而除了 $row=0$ 的这个杯子中的香槟是直接来自于外部，其他杯子中的香槟均来自于它的上一层的相邻的一个或两个杯子中溢出的香槟。根据这个思路，可以求出每一层的每一只杯子中的香槟体积。求出 $row=query\_row$ 的杯子的香槟体积后，取第 $query\_glass$ 个杯子中的体积，并与 $1$ 求最小值返回。
