### [最小绝对差](https://leetcode.cn/problems/minimum-absolute-difference/solutions/1641099/zui-xiao-jue-dui-chai-by-leetcode-soluti-7g0e/)
...
随后我们对数组 $arr$ 进行一次遍历。当遍历到 $arr[i]$ 和 $arr[i+1]$ 时，它们的差为 $\delta =arr[i+1]-arr[i]$。我们使用一个变量 $best$ 存储当前遇到的最小差，以及一个数组 $ans$ 存储答案：

- 如果 $\delta <best$，那么说明我们遇到了更小的差值，需要更新 $best$，同时 $ans$ 清空并放入 $arr[i]$ 和 $arr[i+1]$；
- 如果 $\delta =best$，那么我们只需要将 $arr[i]$ 和 $arr[i+1]$ 放入答案数组即可。
