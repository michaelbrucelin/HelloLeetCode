### [好叶子节点对的数量](https://leetcode.cn/problems/number-of-good-leaf-nodes-pairs/solutions/357905/hao-xie-zi-jie-dian-dui-de-shu-liang-by-leetcode-s/?envType=problem-list-v2&envId=ySsxoJfz)
...
- 时间复杂度：$O(N\cdot distance^2)$，其中 $N$ 为树中节点的数量。对于每个节点，我们至多需要遍历 $O(distance^2)$ 种好叶子节点对。
- 空间复杂度：$O(H\cdot distance)$，其中 $H$ 为树的高度。对于每个节点，我们都需要额外开辟 $O(distance)$ 的空间，而栈的最大深度为 $O(H)$。
