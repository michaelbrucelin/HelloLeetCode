### [最大连续1的个数 III](https://leetcode.cn/problems/max-consecutive-ones-iii/solutions/608931/zui-da-lian-xu-1de-ge-shu-iii-by-leetcod-hw12/?envType=problem-list-v2&envId=h15Gc5qm)
...
$$P[left-1]\ge P[right]-k(1)$$

也就是说，我们需要找到最小的满足 $(1)$ 式的 $left$。由于数组 $A$ 中仅包含 $0/1$，因此前缀和数组是一个单调递增的数组，我们就可以使用二分查找的方法得到 $left$。

**细节**

注意到 $(1)$ 式的左侧的下标是 $left-1$ 而不是 $left$，那么我们在构造前缀和数组时，可以将前缀和整体向右移动一位，空出 $P[0]$ 的位置，即：
