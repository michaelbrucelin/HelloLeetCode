### [通过删除字母匹配到字典里最长单词](https://leetcode.cn/problems/longest-word-in-dictionary-through-deleting/solutions/996014/tong-guo-shan-chu-zi-mu-pi-pei-dao-zi-di-at66/?envType=problem-list-v2&envId=ySsxoJfz)
...
- 时间复杂度：$O(m\times \vert \sum \vert +d\times n)$，其中 $d$ 表示 $dictionary$ 的长度，$\sum $ 为字符集，在本题中字符串只包含英文小写字母，故 $\vert \sum \vert =26$；m 表示字符串 $s$ 的长度，$n$ 表示 $dictionary$ 中字符串的平均长度。预处理的时间复杂度为 $O(m\times \vert \sum \vert )$；判断 $d$ 个字符串是否为 $s$ 的子序列的事件复杂度为 $O(d\times n)$。
- 空间复杂度：$O(m\times \vert \sum \vert )$，为动态规划数组的开销。
