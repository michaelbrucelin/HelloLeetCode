...
> 对于两根柱子 $j_0$ 以及 $j_1$，如果 $j_0<j_1$ 并且 $heights[j_0]\ge heights[j_1]$，那么**对于任意的在它们之后出现的柱子 $i$（$j_1<i$），$j_0$ 一定不会是 $i$ 左侧且最近的小于其高度的柱子**。

换句话说，如果有两根柱子 $j_0$ 和 $j_1$，其中 $j_0$ 在 $j_1$ 的左侧，并且 $j_0$ 的高度大于等于 $j_1$，那么在后面的柱子 $i$ 向左找小于其高度的柱子时，$j_1$ 会「挡住」$j_0$，$j_0$ 就不会作为答案了。

这样以来，我们可以对数组从左向右进行遍历，同时维护一个「可能作为答案」的数据结构，其中按照从小到大的顺序存放了一些 $j$ 值。根据上面的结论，如果我们存放了 $j_0,j_1,\dots ,j_s$，那么一定有 $height[j_0]<height[j_1]<\dots <height[j_s]$，因为如果有两个相邻的 $j$ 值对应的高度不满足 $<$ 关系，那么后者会「挡住」前者，前者就不可能作为答案了。

当我们枚举到第 $i$ 根柱子时，我们的数据结构中存放了 $j_0,j_1,\dots ,j_s$，如果第 $i$ 根柱子左侧且最近的小于其高度的柱子为 $j_i$，那么必然有

$$height[j_0]<height[j_1]<\dots <height[j_i]<height[i]\le height[j_{i+1}]<\dots <height[j_s]$$
...
这样我们在枚举到第 $i$ 根柱子的时候，就可以先把所有高度大于等于 $height[i]$ 的 $j$ 值全部移除，剩下的 $j$ 值中高度最高的即为答案。在这之后，我们将 $i$ 放入数据结构中，开始接下来的枚举。此时，我们需要使用的数据结构也就呼之欲出了，它就是**栈**。

- 栈中存放了 $j$ 值。从栈底到栈顶，$j$ 的值严格单调递增，同时对应的高度值也严格单调递增；
- 当我们枚举到第 $i$ 根柱子时，我们从栈顶不断地移除 $height[j]\ge height[i]$ 的 $j$ 值。在移除完毕后，栈顶的 $j$ 值就一定满足 $height[j]<height[i]$，此时 $j$ 就是 $i$ 左侧且最近的小于其高度的柱子。
...
- 我们枚举 $7$，由于 $6<7$，因此不会移除栈顶元素，所以 $7$ 左侧的柱子是 $6$，位置为 $0$。随后我们将 $7$ 入栈。
  - 栈：`[6(0), 7(1)]`
- 我们枚举 $5$，由于 $7\ge 5$，因此移除栈顶元素 $7$。同样地，$6\ge 5$，再移除栈顶元素 $6$。此时栈为空，所以 $5$ 左侧的柱子是「哨兵」，位置为 $-1$。随后我们将 $5$ 入栈。
  - 栈：`[5(2)]`
...
![](./assets/img/Solution0084_2_01.png)
![](./assets/img/Solution0084_2_02.png)
![](./assets/img/Solution0084_2_03.png)
![](./assets/img/Solution0084_2_04.png)
![](./assets/img/Solution0084_2_05.png)
![](./assets/img/Solution0084_2_06.png)
![](./assets/img/Solution0084_2_07.png)
![](./assets/img/Solution0084_2_08.png)
![](./assets/img/Solution0084_2_09.png)
![](./assets/img/Solution0084_2_10.png)
![](./assets/img/Solution0084_2_11.png)
![](./assets/img/Solution0084_2_12.png)
![](./assets/img/Solution0084_2_13.png)
![](./assets/img/Solution0084_2_14.png)
![](./assets/img/Solution0084_2_15.png)
![](./assets/img/Solution0084_2_16.png)
![](./assets/img/Solution0084_2_17.png)
![](./assets/img/Solution0084_2_18.PNG)
