### [式子变形 + 枚举右维护左（Python/Java/C++/Go）](https://leetcode.cn/problems/longest-balanced-substring-ii/solutions/3803790/shi-zi-bian-xing-mei-ju-you-wei-hu-zuo-p-slnc/)
...
仿照 $525$ 题的做法，设 $a$ 在这个组的**个数前缀和**数组为 $S_a$，$b$ 在这个组的个数前缀和数组为 $S_b$，$c$ 在这个组的个数前缀和数组为 $S_c$。

子串 $[l,r)$ 中的字母 $a,b,c$ 的出现次数相等，可以拆分为如下两个约束：

1. 子串 $[l,r)$ 中的字母 $a$ 和 $b$ 的出现次数相等。
2. 子串 $[l,r)$ 中的字母 $b$ 和 $c$ 的出现次数相等。

只要满足这两个约束，由等号的传递性可知，子串 $[l,r)$ 中的字母 $a$ 和 $c$ 的出现次数相等，即三个字母的出现次数都相等。

两个约束即如下两个等式

$$S_a[r]-S_b[r]=S_a[l]-S_b[l] \\ S_b[r]-S_c[r]=S_b[l]-S_c[l]$$
...
[本题视频讲解](https://leetcode.cn/link/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1FJ4uz1EkN%2F%3Ft%3D5m42s)，欢迎点赞关注~
