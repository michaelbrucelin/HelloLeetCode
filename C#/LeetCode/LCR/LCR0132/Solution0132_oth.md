### [LCR 132. 砍竹子 II（快速幂求余，清晰图解）](https://leetcode.cn/problems/jian-sheng-zi-ii-lcof/solutions/106190/mian-shi-ti-14-ii-jian-sheng-zi-iitan-xin-er-fen-f/)
...
1. 当 $n\le 3$ 时，按照规则应不切分，但由于题目要求必须剪成 $m>1$ 段，因此必须剪出一段长度为 $1$ 的竹子，即返回 $n-1$。
...
![](./assets/img/Solution0132_oth.png)
...
根据求余运算性质推出（$\because$ 本题中 $x<p$，$\therefore$ $x\odot p=x $）：

$$xa^\odot p=[(x^{a-1}\odot p)(x\odot p)]\odot p=[(x^{a-1}\odot p)x]\odot p$$
...
$$x^a\odot p=\begin{cases}(x^2\odot p)^{a//2}\odot p & , a 为偶数 \\ [(x\odot p)(x^{a-1}\odot p)]\odot p=[x(x^2\odot p)^{a//2}]\odot p & , a 为奇数\end{cases}$$
...
| 2 | $3\times (9^9\odot p)$ | $3=1\times 3\odot p$ | $9=3^2\odot p$ | $9=19//2$ |
...
- **时间复杂度 $O(\log N)$：** 其中 $N=a$，二分法为对数级别复杂度，每轮仅有求整、求余、次方运算。
    - [求整和求余运算](https://leetcode.cn/link/?target=https%3A%2F%2Fstackoverflow.com%2Fquestions%2F35189851%2Ftime-complexity-of-modulo-operator-in-python)：资料提到不超过机器数的整数可以看作是 $O(1)$；
    - [幂运算](https://leetcode.cn/link/?target=https%3A%2F%2Fstackoverflow.com%2Fquestions%2F32418731%2Fjava-math-powa-b-time-complexity)：查阅资料，提到浮点取幂为 $O(1)$。
