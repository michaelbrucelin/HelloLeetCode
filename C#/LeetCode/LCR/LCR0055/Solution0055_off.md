### [二叉搜索树迭代器](https://leetcode.cn/problems/kTOapQ/solutions/1038295/er-cha-sou-suo-shu-die-dai-qi-by-leetcod-hwfe/?envType=problem-list-v2&envId=ySsxoJfz)
...
- 时间复杂度：显然，初始化和调用 $hasNext()$ 都只需要 $O(1)$ 的时间。每次调用 $next()$ 函数**最坏情况下**需要 $O(n)$ 的时间；但考虑到 $n$ 次调用 $next()$ 函数总共会遍历全部的 $n$ 个节点，因此总的时间复杂度为 $O(n)$，因此单次调用平均下来的**均摊复杂度**为 $O(1)$。
