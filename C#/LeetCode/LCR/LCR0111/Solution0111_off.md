### [计算除法](https://leetcode.cn/problems/vlzXQL/solutions/1036341/ji-suan-chu-fa-by-leetcode-solution-p731/)
...
**当查询节点 $x$ 父亲时**，如果 $f[x]\ne x$，我们需要先找到 $f[x]$ 的父亲 $father$，并将 $f[x]$ 更新为 $father$。此时，我们有

$$\begin{array}{rcl}
    w[x] & \leftarrow & \dfrac{v[x]}{v[father]} \\
         & = & \dfrac{v[x]}{v[f[x]]} \cdot \dfrac{v[f[x]]}{v[father]} \\
         & = & w[i]\cdot w[f[x]]
\end{array}$$
...
$$\begin{array}{rcl}
    w[f_x] & \leftarrow & \dfrac{v[f_x]}{v[f_y]} \\
           & = & \dfrac{v[x]/w[x]}{v[y]/w[y]} \\
           & = & \dfrac{v[x]}{v[y]}\cdot\dfrac{w[y]}{w[x]}
\end{array}$$

也就是说，当在已有的图中添加一条方程式 $\dfrac{v[x]}{v[y]}=k$ 时，需要将 $w[f_x]$ 更新为 $k\cdot\dfrac{w[y]}{w[x]}$。
