### [含有所有字符的最短字符串](https://leetcode.cn/problems/M1oyTv/solutions/1417786/han-you-suo-you-zi-fu-de-zui-duan-zi-fu-a8q50/?envType=problem-list-v2&envId=ySsxoJfz)
...
- 时间复杂度：最坏情况下左右指针对 $s$ 的每个元素各遍历一遍，哈希表中对 $s$ 中的每个元素各插入、删除一次，对 $t$ 中的元素各插入一次。每次检查是否可行会遍历整个 $t$ 的哈希表，哈希表的大小与字符集的大小有关，设字符集大小为 $C$，则渐进时间复杂度为 $O(C\cdot \vert s\vert +\vert t\vert)$。
