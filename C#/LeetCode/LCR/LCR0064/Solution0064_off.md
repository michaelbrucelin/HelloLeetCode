### [神奇的字典](https://leetcode.cn/problems/US1pGT/solutions/1656424/shen-qi-de-zi-dian-by-leetcode-solution-uvh3/)
...
在查询时，我们可以使用递归 $+$ 回溯的方法，使用递归函数 $dfs(node,pos,\bmod ified)$，其中的变量分别表示：当前遍历到的字典树上的节点是 $node$ 以及待查询字符串 $searchWord$ 的第 $pos$ 个字符，并且在之前的遍历中是否已经替换过恰好一个字符（如果替换过，那么 $\bmod ified$ 为 $true$，否则为 $false$）。

如果 $node$ 有一个值为 $searchWord[pos]$ 的子节点，那么我们就可以继续进行递归。同时，如果 $\bmod ified$ 为 $false$，我们可以将 $searchWord[pos]$ 替换成任意一个是 $node$ 子节点的字符，将 $\bmod ified$ 置为 $true$ 并继续进行递归。

当 $pos$ 等于 $searchWord$ 的长度时，说明递归完成。此时我们需要检查 $node$ 是否是一个字典树上的结束节点（即一个单词的末尾），同时需要保证 $\bmod ified$ 为 $true$，因为我们必须进行一次修改。
